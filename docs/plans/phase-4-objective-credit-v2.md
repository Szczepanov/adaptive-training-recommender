# Phase 4 â€” Objective credit V2, and honest load

* **Status:** In progress â€” tasks 4.1â€“4.3 implemented on `gemini-phase4`; 4.5 is active
* **Blocked by:** Phase 0â€™s harness-gate evidence is required before credit/fusion cutover is
...
* **Unlocks:** Phase 5
* **Addresses:** F7, F8, F12
* **Rough effort:** 4â€“5 days

---

## Task board

Status legend: `[ ]` not started Â· `[-]` in progress Â· `[x]` finished.
...
vocabulary rename, and make delivered load respond to what was actually done â€” without
...
| Stimulus-vector coverage â‰Ą 0.6 | `microcycle.ts` `creditObjectivesFromStimulus` | live (primary) |
...
`completedCredit` / `projectedCredit` / `windowStart` / `windowEnd` / `priority` â€” all
already declared in `models.ts` and all unused â€” carrying real values from Phase 2's
`PlanDefinition`.
...
A single generic formula is not appropriate â€” 15 abbreviated minutes should not equal a
...
| `threshold_quality` | accumulated work near threshold â€” `2Ă—8`, `4Ă—8`, `3Ă—12` are not equivalent |
...
shared formula with per-objective coefficients â€” that reintroduces F11's uncited-constant
...
that silently ignores a factor its own specification names â€” that is how an unfalsifiable
...
Consumers disagree on which vocabulary is authoritative â€” `optimizer.ts` reads legacy
...
1. Make canonical axes **required**; delete the legacy aliases. **Inventory every producer
   and consumer first** â€” the codemod is not limited to `templates.ts` and the catalog.
   ...
   **Specify that compatibility read before deleting anything.** Persisted
   ...
   cannot codemod, and records exist in three states â€” canonical-only, legacy-only, and
   ...
   | Both present and disagreeing | Canonical wins, and the divergence is **logged** â€” it means a writer was missed, and silently preferring one would hide that. |
   ...
   The canonical mapping is the *identity for the five renamed axes* â€” this is a rename,
   not a remodelling â€” with one exception: the two derived fallbacks
   (`vo2MaxPower â† surgeRepeatability * 0.8`, `fatigueResistance â† thresholdDevelopment
...
3. **Decision (2026-08-08): delete the derived fallbacks; make templates declare the axes
   ...
   The alternative â€” find a citation for the coefficients â€” is the wrong shape of work.
...
## `[x]` 4.3 â€” F12: fatigue, in two separable pieces
...
criticises. It may also be wrong â€” internal response (HRV/RHR/soreness) is partly a
...
### (a) Correctness â€” do now, no modelling judgement required
...
**Sort, then assert â€” do not take the dashboard down.** History is external, persisted
...
error, not bad data â€” but the caller still degrades to the established controlled state
...
### (b) Modelling â€” do not pre-decide

Two real questions:

* **Saturation.** `Math.min(1, ...)` per axis (`applyCompletedSessionLoad`) means two hard
  lower-body days â‰ one, at the ceiling. The model cannot represent "significantly deeper
  in the hole".
...
before committing to one â€” and record the comparison in an ADR, with the data.
...
## `[x]` 4.5 â€” `PlannedDose`: give `intensityScale` its consumer (D2)
...
```text
base session cost Ă— delivered dose Ă— measured-response adjustment
```

Keep the six dimensions â€” they are a good abstraction. This does **not** require adopting
...
â€” enough for `base Ă— delivered dose`, and nothing at all for the measured-response
...
1. **Duration and completion â€” now.** Extend the cost function to consume `DeliveredDose`
   as it stands. This alone fixes the stated defect: a 40-minute and a 3-hour hard ride
   stop scoring alike.
2. **Measured response â€” only once it has a source.** The signal exists upstream
   ...
   the sessions that have no measurement â€” an untracked activity has no training effect,
...
(Phase 5) â€” 4.x improves the vectors, not the inference chain that produces them.

## Docs to update

* **ADR-0014** (new) â€” objective credit V2 semantics and the fatigue fusion decision, with
  the harness comparison attached
* **ADR-0006** â€” amend: strain telemetry interacts with the new credit model
* `docs/architecture/recommendation-engine.md` â€” the credit and fatigue sections
