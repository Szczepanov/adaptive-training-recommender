...
* The import schema has been exercised at least once end to end: a real plan generated by
  the athlete's own AI from the published prompt block, reviewed by hand for schema
  conformance. **Do not start 8.1 before this.** A schema that has never met a real
  generated plan will be wrong in ways review does not surface, and every later work item
  inherits its shape.
