...
- [x] `npm run simulate:diff` reports no changed pre-existing baseline scenario. *(It also reports 11 `[NEW SCENARIO]` lines. Those predate this phase — the committed baseline was never refreshed after Phase 7 added scenarios — and are identical before and after this branch. No `[MODIFIED]` or `[REMOVED SCENARIO]` line appears.)*
- [x] A plan generated by the athlete's own AI from the published prompt block imports without hand-editing. *(Evidence: the 2026-08-15 round-trip recorded in [`external-plan-schema.md`](../external-plan-schema.md#round-trip-result-2026-08-15) — 21 sessions, zero hard schema errors. The three revisions it forced are folded into the contract.)*
- [x] An imported session that fails a hard gate is never displayed as actionable, and names its failing gate. *(`externalSession.test.ts`; `ExternalVerdictBanner` renders the gate in plain language and suppresses the prescription.)*
- [x] A free-text imported fallback is never displayed as actionable; any actionable replacement has independently passed the normal gates.
- [x] An imported session receives strictly less objective credit than the same session as an exact catalog match. *(`externalTemplateNeverInCatalog.test.ts`, via `stimulusConfidenceForTier('authoredExternal') === 'inferred'`.)*
- [x] A missed session produces a proposal, never a silent re-placement. *(`externalPlacement.test.ts`; `applyConfirmedProposal` is the only writer and `ExternalPlanWeek` calls it only from a confirmed action.)*
- [x] A re-import supersedes forward only; a previously adjudicated day's persisted recommendation and audit are byte-identical after it. *(`externalPlanService.test.ts` asserts the exact write set: the new revision and the header, nothing else.)*
- [x] An external recommendation replays reproducibly; a mutated revision fails replay with a hash-mismatch reason. *(`replay.test.ts`.)*
- [x] `check-policy-drift.mjs` passes with the bumped `POLICY_VERSION`.
- [x] Travel dose reduction is applied exactly once (by the `AuthoredPlanBlock`), verified on a fixture where a travel block overlaps imported sessions. *(`externallyPlannedMode.test.ts`.)*
...
* [x] ADR-0019 → `Accepted` once the design is agreed.
* [x] `architecture/recommendation-engine.md` — a third planning mode and the adjudication
      path; the authority ordering gains no new step but now has a second entry point.
* [x] `external-plan-schema.md` — drop the proposal banner; record any schema change forced
      by the precondition round-trip.
* [x] `docs/README.md` and `plans/README.md` — index rows and decision-register entries.
* [x] `AGENTS.md` — `engine/` package map gains `externalSession.ts`, `externalPlacement.ts`,
      `externalCritique.ts`.
...
| 8.6 | Planning-mode wiring | `[x]` | — |
| 8.7 | Weekly critique layer | `[x]` | 8.6 |
| 8.8 | Provenance and replay | `[x]` | 8.6 |
| 8.9 | UI | `[x]` | 8.6 (import screen may start after 8.1) |
