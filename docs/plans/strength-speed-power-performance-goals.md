...
- **POLICY_VERSION bumped** to `2026-09-performance-goal-demand-projection-v1` because `adapters.ts` is on `check-policy-drift.mjs`'s `decisionAffectingFiles` list, even though this change is behaviorally inert. `simulate:diff` showed no change, but that alone is a weak signal since the committed simulation fixtures contain no goal with a typed `performanceTarget` — the real proof of inertness is the structural architecture test above plus `adapters.test.ts`'s byte-identical-`UserContext`-except-one-field test.
...
- **Canonical-definition coverage is candidate coverage, not delivered-variant proof.** `workoutProvidesDirectCoverage` intentionally classifies the authored `WorkoutDefinition.blocks` used for catalog/weekly-allocation candidate identity; it does not apply `WorkoutVariant.stepOverrides`. That distinction matters already: `strength_full_body_maintenance_01`'s `return_to_training` variant omits `front_squat` even though the canonical workout contains it. PG7 must therefore verify the resolved prescription/variant still contains the direct-coverage step before marking a performance-target occurrence fulfilled (otherwise emit the relevant PG5.4 shortfall); the PG5.2 helper alone must never be used as delivered-work evidence.
