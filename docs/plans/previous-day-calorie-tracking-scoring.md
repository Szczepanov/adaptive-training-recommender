...
- [ ] Define `NutritionTrackingAdherence` type:
  `'fully_tracked' | 'mostly_tracked' | 'minimal' | 'untracked' | 'fasted'`
  in `app/src/engine/models.ts`.
- [ ] Add `nutritionAdherenceYesterday?: NutritionTrackingAdherence | null;` to `DailySubjectiveCheckin`.
- [ ] Ensure docstrings explicitly document zero recommendation authority per ADR-0042 `D-NUT-AUTH`.

### S1: Validation & Persistence
- [ ] Add validation for `nutritionAdherenceYesterday` in `app/src/engine/validationCore.ts`:
...
- [ ] Add parser support in `app/src/persistence/parsers/decisionInputs.ts`:
...
- [ ] Update `CheckinService.upsertCheckin` in `app/src/services/checkinService.ts`:
...
- [ ] Create `app/src/components/checkin/NutritionAdherenceSection.tsx` and `.css`:
  - Present options: Fully Tracked, Mostly Tracked (~75%), Minimally Tracked (<50%), Untracked, Fasted (0 kcal).
...
- [ ] Wire `NutritionAdherenceSection` into `DailyCheckin.tsx` below `HungerSection`.
- [ ] Add unit tests for `NutritionAdherenceSection` in `NutritionAdherenceSection.test.tsx`.

### S4: Nutrition View Integration
- [ ] Update `NutritionPanel.tsx` to read yesterday's checkin adherence (or accept as part of day view).
- [ ] Render adherence badges:
  - `fasted`: render "Deliberate Fast (0 kcal)" instead of "Unlogged".
...
- [ ] Verify `app/src/nutrition/__tests__/engineIsolation.test.ts` passes (zero engine imports of nutrition).
- [ ] Verify `app/src/nutrition/__tests__/nutritionRecommendationInvariance.test.ts` passes and add tests proving that `nutritionAdherenceYesterday` values do not alter engine recommendations.

### S6: Full Suite & PR
- [ ] Run full test suites: `make check`, `npm test`, `npm run test:rules`.
- [ ] Run code-reviewer subagent and address findings.
