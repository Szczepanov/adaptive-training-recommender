...
| HRF | [Activity heart-rate measurement fidelity](./activity-heart-rate-measurement-fidelity.md) | **Approved** | HRF7 | HRF6 inventories all current HR consumers and adds a shadow adapter for vendor HR-derived Training Load; HRF7 adds replay observability, while production activation still requires HRF8 replay/paired evidence and a separate activation decision | sensor/source-aware activity HR confidence, artifact diagnostics, per-use authority, and evidence-gated protection of HR-derived decisions |
