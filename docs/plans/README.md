...
Rows G, S, M, CT, UX, OV, HA, SV, MS, and PI are **not phases**. They are capability/surface plans whose work items are
prefixed `G*`, `S*`, `M*`, `CT*`, `UX*`, `OV*`, `HA*`, `SV*`, `MS*`, `PI*` precisely so they cannot be mistaken for the `Phase 0`–`9`
