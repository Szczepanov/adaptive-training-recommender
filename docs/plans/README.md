...
| SAW | [Server-authoritative anthropometry writes](./anthropometry-server-authoritative-writes.md) | **In progress** | none | ADR-0040's trusted mutation boundary | replaces direct Firestore anthropometry mutations with a verified Cloud Run API after a valid ten-measurement payload hit Firestore Rules' 1,000-expression ceiling; owner reads remain direct and recommendation authority remains zero |


Rows G, HRF, S, M, CT, WU, UX, OV, HA, SV, MS, PI, ES, TO, SEP, SKR, and BC are **not phases**. They are capability/surface plans whose work items are
prefixed `G*`, `HRF*`, `S*`, `M*`, `CT*`, `WU*`, `UX*`, `OV*`, `HA*`, `SV*`, `MS*`, `PI*`, `ES*`, `TO*`, `SEP-*`, `SKR*`, `BC*` precisely so they cannot be mistaken for the `Phase 0`–`9`
