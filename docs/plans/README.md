...
**Draft**: HRF0's real-account FIT provenance/decoder spike is the only startable item; all
schema/authority work waits for that evidence and acceptance of proposed ADR-0031, and
production gating remains separately evidence-gated after replay.
...
| HRF | [Activity heart-rate measurement fidelity](./activity-heart-rate-measurement-fidelity.md) | **Draft** | HRF0 | HRF1+ require HRF0 plus acceptance of proposed ADR-0031; production activation requires HRF8 replay/paired evidence and a separate activation decision | sensor/source-aware activity HR confidence, artifact diagnostics, per-use authority, and evidence-gated protection of HR-derived decisions |
...
| ES | [Direct Eight Sleep recovery ingestion](./eight-sleep-direct-recovery-ingestion.md) | **In progress (default-off)** | ES9 (`backfill-eight-sleep-direct` now daily-scheduled once secrets are deployed; `compare-eight-sleep-transports` run on demand once an accumulation window has elapsed) | ES10 needs ES9's accumulated evidence plus a separate activation review | owned direct read-only recovery connector for Eight Sleep (ADR-0030); ES8's real-account probe ran 2026-08-28; ES9's persistence/comparator tooling implemented |
