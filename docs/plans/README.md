...
| HRF | [Activity heart-rate measurement fidelity](./activity-heart-rate-measurement-fidelity.md) | **Approved** | HRF6 | HRF5 now provides a shadow-only per-use authority helper; HRF6 must audit and adapt every HR consumer before HRF7 replay observability, while production activation still requires HRF8 replay/paired evidence and a separate activation decision | sensor/source-aware activity HR confidence, artifact diagnostics, per-use authority, and evidence-gated protection of HR-derived decisions |
