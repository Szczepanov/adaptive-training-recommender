...
complete; Performance outcome validation (OV) is **Draft** and blocked by design review.
...
2026-08-21; its implementation is delegated to the Draft OV plan and remains blocked on
design review rather than becoming automatic continuation after M5. M8 may consume M6/M7
only when each capability is independently justified and available; M9 remains behind its
own named triggers.
...
| M | [Multidomain session authoring, execution & evidence](./multidomain-session-authoring-execution-and-evidence.md) | **In progress** | none | M6 still requires an explicit real-use trigger; M7's trigger is satisfied but implementation is delegated to Draft OV and blocked on design review; M8 is evidence-gated; M9 needs its own named triggers | source-neutral authored sessions, safe mixed-dose execution and occurrence-linked response first; specialized field/testing capability only when athlete use proves the generic runner/evidence model insufficient |
...
| OV | [Performance outcome validation & goal-progress loop](./performance-outcome-validation.md) | **Draft** | none | design review of this plan | activates M7 — the repeated-testing usage trigger fired 2026-08-21 — with a bounded evidence-only outcome/progress/block-report capability; not an original review finding |
