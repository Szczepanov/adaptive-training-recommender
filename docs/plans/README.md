...
| WU | [Structured strength warm-ups](./structured-strength-warmups.md) | **In progress** | — | catalog-specific visual capture and full host-window verification | adds explicit, executable, correctly logged warm-ups to every active catalog strength session |
...
| SAW | [Server-authoritative anthropometry writes](./anthropometry-server-authoritative-writes.md) | **In progress** | deployment/completion verification | ADR-0040 is accepted; the delivery-contract evidence is not yet recorded in the plan | implementation landed in #572: authenticated Cloud Run write path, fail-closed Rules, conformance tests, and deployment wiring; live deployment/completion remains to be verified, owner reads remain direct, and recommendation authority remains zero |
