...
| HRF | [Activity heart-rate measurement fidelity](./activity-heart-rate-measurement-fidelity.md) | **Approved** | HRF5 | HRF4 now persists compact shadow-only fidelity evidence; HRF5 adds the per-use TypeScript authority engine, while production activation still requires HRF8 replay/paired evidence and a separate activation decision | sensor/source-aware activity HR confidence, artifact diagnostics, per-use authority, and evidence-gated protection of HR-derived decisions |
