...
the implementation is merged in [PR #471](https://github.com/Szczepanov/adaptive-training-recommender/pull/471).

The latest full-plan audit is [`2026-09-17-plan-status-reconciliation.md`](../analysis/2026-09-17-plan-status-reconciliation.md).
...
The plan for GitHub issue [#746](https://github.com/Szczepanov/adaptive-training-recommender/issues/746)
(Day-2 strength after lower-body strain; Week-2 strength under travel) is
[`2026-09-24-issue-746-strength-sequencing-and-travel-week-credit.md`](./2026-09-24-issue-746-strength-sequencing-and-travel-week-credit.md) — **Implemented**,
with decisions D1–D4 delivered (`POLICY_VERSION = 2026-09-strength-deferral-forecast-credit-aging-v1`) and the WP3.0
[forecast-credit parity analysis](../analysis/2026-09-24-issue-746-forecast-credit-parity.md).

The [strength, speed and power performance-goals plan](./strength-speed-power-performance-goals.md) is **In progress**, sourced from the [2026-09-19 performance-goal gap analysis](../analysis/2026-09-19-strength-speed-power-performance-goal-gap.md). Stage 1 (PG0–PG4.5) is **Implemented** under [ADR-0041](../adr/0041-strength-speed-power-performance-goals.md): a typed metric + canonical subject/test contract for one strength (`conventional_deadlift` 1RM), one speed (`sprint_10m_standing-r1`) and one power (`cycling_5s_peak_power-r1`) target, athlete-facing target UX, honest current-evidence/progress projection, and an advisory goal-feasibility layer (strength family only; speed/power report `insufficient_evidence` pending reviewed rate-of-change evidence) that reports plausibility, confidence, required change, capacity factors and evidence provenance without rewriting the athlete's goal or becoming prescription authority. It has **no recommendation authority yet**. PG5.1 (a typed `PerformanceGoalDemand` projection threaded into `UserContext`) and PG5.2 (a reviewed planning-rule registry/coverage classifier in `performanceGoalPlanningRules.ts`, keyed to `WorkoutDefinition.blocks[].steps[].exerciseId`) are now **Implemented**, but both are deliberately unread by any planning code — `performanceGoalDemand.architecture.test.ts` and `performanceGoalPlanningRules.architecture.test.ts` each enforce that structurally. PG5.2's classifier tests also confirmed two concrete catalog gaps against the live workout catalog: no active workout gives direct coverage to `conventional_deadlift` or to the cycling 5 s peak-power exercise `bike_sprint_power`. PG5.3–PG5.4's live wiring, PG6 (catalog audit — now scoped by PG5.2's confirmed gaps), PG7 (weekly-allocation authority wiring), PG8 (formal outcome evaluation) and PG9 (progression automation) remain Draft/not started and require their own policy verification, `POLICY_VERSION` bump and simulation review before typed performance targets can affect weekly allocation or session selection. A recommended sequencing for that remaining work, plus a real open design question in `weeklyAllocation.ts`'s reservation search that needs its own ADR before PG7, is recorded in [the Stage 2 weekly-allocation integration-points analysis](../analysis/2026-09-19-stage2-weekly-allocation-integration-points.md).
...
9.0.1 operational precondition closed 2026-08-27; 9.0.7 remains startable, but its
historical 2026-08-22 target passed without a recorded completed evidence block.
...
Physiological identity passport & measurement trust (PI) is **In progress**:
...
**In progress**: HRF0–HRF7 are complete as an additive/shadow stack; HRF8 replay plus
independent paired-reference evidence remains the ship/no-ship gate, and production authority
still requires a separate activation decision. Training occurrence reconciliation (TO) is **in
progress**: ADR-0034's canonical `PerformedTrainingOccurrence` domain, gated Activities read
model, durable performed-rest timing, history shadow diff, and FIT workout-identity decoding
all shipped in
[PR #324](https://github.com/Szczepanov/adaptive-training-recommender/pull/324), merged
2026-09-02. Every surface landed shadow-mode or flag-gated off by default, per the same
evidence-first discipline as D-MPOLICY; weekly coverage credit has since been cut over to
canonical occurrence facts in
[PR #331](https://github.com/Szczepanov/adaptive-training-recommender/pull/331), so
`trainingIntent.ts` reads canonical facts in the live path while TO2's Activities read
model remains flag-gated off by default (the frontend deploy forwards the
`VITE_TRAINING_OCCURRENCE_ACTIVITIES_POLICY` repository secret; `canonical-v1` enables it).
...
value-bearing work is operational OV7 on the real event/block timeline. PR #631 delivered the
event-aware capture infrastructure; `OV7.1` remains open until a real event outcome is captured
and included in a block report. `OV4.4` remains gated
...
| PG | [Strength, speed and power performance goals](./strength-speed-power-performance-goals.md) | **In progress** | PG5.3–PG5.4 design/implementation; PG0–PG4.5 first slice and PG5.1–PG5.2 are implemented | PG4.5.4 needs sufficient personal longitudinal evidence; PG6 waits on reviewed knowledge for the two catalog gaps PG5.2 confirmed; **PG5.3 implementation itself waits on PG6 landing real coverage for a given target** (wiring dose refinement before real coverage exists would regress a currently-satisfiable requirement) — PG5.3 design may proceed in parallel; PG7 waits on PG5.3+PG6 and a dedicated `weeklyAllocation.ts` priority-tier ADR; PG8/PG9 follow the plan's evidence/usage gates | typed measurable strength/speed/power targets, protocol-aware progress, advisory feasibility, and later target-specific planning without using target values as dose |
...
| HRF | [Activity heart-rate measurement fidelity](./activity-heart-rate-measurement-fidelity.md) | **In progress** | HRF8 | HRF8 replay/paired evidence and a separate activation decision remain required; HRF9/HRF11 follow activation or an explicit shadow closeout | sensor/source-aware activity HR confidence, artifact diagnostics, per-use authority, and evidence-gated protection of HR-derived decisions |
...
| M | [Multidomain session authoring, execution & evidence](./multidomain-session-authoring-execution-and-evidence.md) | **In progress** | none startable (M8.1 done) | M6 still requires an explicit real-use trigger; repeated-testing implementation has transferred to OV and is not an M blocker/task; M8.2 needs real history and only independently justified M6/OV evidence if required; M8.3 needs sufficient measured-candidate evidence; M9 needs its own named triggers | source-neutral authored sessions, safe mixed-dose execution and occurrence-linked response; specialized field work remains usage-triggered, while repeated testing/progress is owned by OV |
...
| E2E | [CI browser E2E tests for core athlete journeys](./ci-browser-e2e-tests.md) | **Implemented** | — | — | adds Auth/Firestore-emulator browser coverage for sign-in, daily decision, session lifecycle, and duplicate session starts |
| WU | [Structured strength warm-ups](./structured-strength-warmups.md) | **Implemented** | — | — | adds explicit, executable, correctly logged warm-ups to every active catalog strength session; catalog-specific desktop/mobile visual coverage is verified |
...
| UX-R | [Mobile UX remediation: interaction, overlays, and navigation](./2026-09-mobile-ux-remediation.md) | **In progress** | manual keyboard/screen-reader and real-device spot checks; PR review | issues #727–#732 are implemented in the #734 sequence; frontend check, 18 browser E2E tests, and visual captures pass, with remaining evidence recorded in the plan | improves mobile interaction targets, overlays, structured sessions, and history-aware navigation without changing recommendation policy |
| OV | [Performance outcome validation & goal-progress loop](./performance-outcome-validation.md) | **In progress** | OV7.1 real event capture, then OV7.2–OV7.4; OV4.4/OV6.2 only when their triggers are met | OV7 follows the event/block timeline; OV4.4 needs close-spaced repeats; OV6.2 needs repeated report use; OV8 needs multiple prospective blocks | sole implementation/status owner of repeated testing/progress; OV7 capture infrastructure merged in #631, with production selection authority still explicitly excluded |
| HA | [Health anomaly and possible-illness alerting](./health-anomaly-and-illness-risk-alerting.md) | **In progress** | HA7 evidence & prospective accumulation; HA9-R3 evidence collection | HA6.4 needs enough labelled personal history; HA7 needs real replay/prospective evidence; HA8 remains release-gated | explainable physiological-anomaly evidence with structured confounders; HA0–HA5 on `main` (HA5 via #171), HA6.1–HA6.3 prospective labels merged via PR #174; HA9-R0–R2 ([respiration-elevation training tightening](./respiration-elevation-training-tightening.md)) merged via PR #306, HA9-R3 is collecting evidence, not a parallel status board |
...
| MS | [Multisource health & recovery ingestion](./multisource-health-and-recovery-ingestion.md) | **In progress** | MS17 | MS17 needs Google Restricted Scope App Verification/CASA for Google Health transport, verified in-app health-data disclosure and explicit consent, sufficient prospective/incremental evidence, and a rollback flag | source-aware observation contract, Google Health ingestion, Eight Sleep export probing, and evidence-gated fusion (ADR-0027); MS0–MS16, MS18, MS19 done, only MS17's production-activation decision remains |
| PI | [Physiological identity passport & measurement trust](./physiological-identity-passport-and-measurement-trust.md) | **In progress** | PI8 prospective label accumulation | PI9's activation decision needs prospective label evidence that doesn't yet exist; PI10's telemetry additionally needs an analytics pipeline this application does not have | provider-neutral identity-attribution/measurement-trust layer (ADR-0028) between shared-source observations and baseline/fusion logic; PI0-PI7 implemented, PI8/PI9/PI10 partial |
...
| TO | [Training occurrence reconciliation & structured strength unification](./training-occurrence-reconciliation-and-strength-session-unification.md) | **In progress** | [PR #324](https://github.com/Szczepanov/adaptive-training-recommender/pull/324) (all five scoped PRs) merged 2026-09-02; [PR #331](https://github.com/Szczepanov/adaptive-training-recommender/pull/331) then cut weekly coverage credit over to canonical occurrence facts | activation of TO4 (history shadow diff feeding real evidence) and TO5 (FIT workout-identity in reconciliation scoring) needs reviewed shadow data per ADR-0034, same discipline as D-MPOLICY | canonical `PerformedTrainingOccurrence` distinct from `SessionOccurrence`'s planning authority (ADR-0034); TO1 reconciliation engine + Firestore collections are shadow-mode only, TO2's Activities read model is flag-gated off by default (production enables it with the `VITE_TRAINING_OCCURRENCE_ACTIVITIES_POLICY=canonical-v1` repository secret, which the frontend deploy now forwards) and its Completed Workout card renders per-set prescribed/performed load, logged effort, actual vs. target rest, watch HR summary/zones and a manual structured↔Garmin link action, TO3 adds durable performed-rest timing, TO4/TO5 are deliberately decode/shadow-only, not wired into any live recommendation or coach path |
| SH | [Strength session HR per set and per rest](./strength-session-hr-alignment.md) | **Draft** | SH4 (pure alignment against a fixture series) | D1 needs an ADR-0026 amendment before SH2/SH3 persist any HR series | display-only per-set HR and rest HR recovery on the linked Completed Workout card; no decision authority |
...
| SKR | [Sports Knowledge Registry](./sports-knowledge-registry-follow-up.md) | **In progress (SKR1–SKR5 complete; SKR6 planned)** | none | SKR6 waits until the registry creates enough demand | ADR-0033 versioned claim registry, persisted recommendation lineage (SKR1), engine knowledge coverage inventory (SKR2), heuristic policy migrations & catalog recovery audit (SKR3), identity-scoped athlete-specific evidence boundary with safety monotonicity (SKR4), and deterministic freshness governance/reporting with no per-claim data migration (SKR5) |
| BC | [Body-composition and fueling observations](./body-composition-and-fueling-observations.md) | **Implemented (BC0–BC4; BC5 skipped)** | none | none | protocol-aware tape measurements, provider-first body mass, optional hunger 1–10 check-in, and retrospective trends in Data with zero recommendation authority; two known open items (both-sides-in-one-session limb entry, no dedicated DST-boundary test for this feature's trend windows) are noted in the plan's completion summary |
| NTA | [Previous-day calorie tracking scoring](./previous-day-calorie-tracking-scoring.md) | **In progress** | CI/merge closure | current PR #710 verification | optional D-1 behavioral logging-quality context, strict missing-vs-zero semantics, validated retrospective D+1 reads, and full-day-fast contradiction visibility; ADR-0042 recommendation authority remains zero |
...
| #806 | [Evidence-backed weekly aerobic dose envelope](./2026-09-26-issue-806-weekly-aerobic-dose.md) | **In review** | `make verify`, simulation/persona gates and independent diff review | deterministic gates and review, not a design decision | 28-day history-derived easy-aerobic weekly envelope with 150/150/300 health fallback, no intensity equivalence, and a conditional exact long aerobic anchor; distinct from #757's single-session coverage floor |
