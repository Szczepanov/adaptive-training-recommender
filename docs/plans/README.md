...
| M | [Multidomain session authoring, execution & evidence](./multidomain-session-authoring-execution-and-evidence.md) | **In progress** | M0.1–M0.3, M1.1–M1.7 | M2+ also require the successor ADR from M0.1 | source-neutral authored sessions, safe mixed-dose execution, occurrence-linked response, and protocol-aware field/speed/power evidence — ordered so every milestone from M1 ends with a phone-usable capability, and so the session→response evidence link ships with the M1 Strength repairs rather than five milestones later |
