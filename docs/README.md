...
* [**ADR-0017: Training Intent Profile & First-Class Planning Modes**](./adr/0017-training-intent-and-planning-modes.md) — Accepted evidence-to-dose-to-capacity Evergreen contract that preserves structured and demand-derived event paths (Phase 7B).
...
* [**Multidomain session authoring, execution & evidence**](./plans/multidomain-session-authoring-execution-and-evidence.md) — *In progress.* M0–M5.3 are delivered. M6 remains independently usage-triggered. M7's repeated-testing trigger fired 2026-08-21: M7.1/M7.2 are delivered through OV0–OV2, M7.3 is next through OV3, and M7.4 still waits for repeated comparable evidence.
...
* [**Performance outcome validation & goal-progress loop**](./plans/performance-outcome-validation.md) — *In progress.* OV0–OV2 are delivered through PRs #154/#155: metric/protocol/comparability contracts, immutable protocol/observation persistence, assessment attempts, manual adapter, ecological competition outcomes, and Firestore rules/emulator coverage. OV3.1 is the next startable item; progress/block-report semantics remain later evidence-only slices.
