...
5. **Firebase Hosting** — compile the production bundle (`npm run typecheck && npm run build:bundle`)
   ...
   verify the `/api/garmin/**` and `/api/anthropometry/**` Hosting rewrites reach their Cloud Run
   services. Unit tests are never re-evaluated against live production credentials during
   deployment.
