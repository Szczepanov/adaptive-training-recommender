...
class EightSleepError(RuntimeError): pass
class EightSleepAuthenticationError(EightSleepError): pass
class EightSleepRateLimitError(EightSleepError): pass
class EightSleepApiError(EightSleepError): pass
class EightSleepSchemaError(EightSleepError): pass
...
class EightSleepToken:
    ...
    def is_valid(self, now: datetime | None = None) -> bool:
        return bool(self.access_token) and (now or datetime.now(UTC)) < self.expires_at - timedelta(seconds=60)
...
class EightSleepHttpTransport(Protocol):
    def request(self, *, method: str, url: str, headers: Mapping[str, str], data: bytes | None, timeout: float) -> EightSleepHttpResponse: ...
    ...
    def request(self, *, method: str, url: str, headers: Mapping[str, str], data: bytes | None, timeout: float) -> EightSleepHttpResponse:
        ...
        try:
            with urlopen(request, timeout=timeout) as response:  # noqa: S310 -- fixed HTTPS API endpoints
                return EightSleepHttpResponse(response.status, dict(response.headers.items()), response.read())
...
class EightSleepClient:
    ...
    def __init__(self, settings: EightSleepSettings, *, transport: EightSleepHttpTransport | None = None, sleep_fn: Any = time.sleep, now_fn: Any = lambda: datetime.now(UTC)) -> None:
        settings.validate()
    ...
    def authenticate(self, *, force: bool = False) -> EightSleepToken:
        if not force and self._token and self._token.is_valid(self._now()):
            return self._token
        data = urlencode({"grant_type": "password", "username": self.settings.email or "", "password": self.settings.password or "", "client_id": self.settings.client_id or "", "client_secret": self.settings.client_secret or ""}).encode()
        response = self.transport.request(method="POST", url=AUTH_URL, headers={"Content-Type": "application/x-www-form-urlencoded"}, data=data, timeout=self.settings.timeout_seconds)
        if response.status in (401, 403):
            raise EightSleepAuthenticationError(f"Eight Sleep authentication rejected credentials (HTTP {response.status}).")
        if response.status == 429:
            raise EightSleepRateLimitError("Eight Sleep authentication was rate limited (HTTP 429).")
    ...
    def get_trends(self, *, from_date: str, to_date: str, timezone: str) -> Any:
        query = urlencode({"from": from_date, "to": to_date, "tz": timezone, "include-main": "false", "include-all-sessions": "true", "model-version": "v2"})
        return self._authorized_json(f"{CLIENT_API_BASE_URL}/users/{self.get_user_id()}/trends?{query}")
    ...
    def _authorized_json(self, url: str) -> Any:
        ...
        while True:
            ...
            response = self.transport.request(method="GET", url=url, headers={"Authorization": f"Bearer {token.access_token}", "Accept": "application/json"}, data=None, timeout=self.settings.timeout_seconds)
            if response.status == 401 and not reauthed:
                self.clear_token(); reauthed = True; continue
            if response.status in (401, 403):
                raise EightSleepAuthenticationError(f"Eight Sleep API authorization failed (HTTP {response.status}).")
            if response.status == 429 or 500 <= response.status < 600:
                if retries >= self.settings.max_retries:
                    if response.status == 429:
                        raise EightSleepRateLimitError("Eight Sleep API remained rate limited after bounded retries.")
                    raise EightSleepApiError(f"Eight Sleep API failed after bounded retries (HTTP {response.status}).")
                self._sleep(_retry_delay(response.headers, retries)); retries += 1; continue
            if not 200 <= response.status < 300:
                raise EightSleepApiError(f"Eight Sleep API request failed (HTTP {response.status}).")
            return _decode_json(response.body, "API")

def _decode_json(raw: bytes, context: str) -> Any:
    try: return json.loads(raw.decode())
    except (UnicodeDecodeError, json.JSONDecodeError) as exc: raise EightSleepSchemaError(f"Eight Sleep {context} response was not valid JSON.") from exc

def _retry_delay(headers: Mapping[str, str], attempt: int) -> float:
    raw = headers.get("Retry-After") or headers.get("retry-after")
    if raw:
        try: return min(60.0, max(0.0, float(raw)))
