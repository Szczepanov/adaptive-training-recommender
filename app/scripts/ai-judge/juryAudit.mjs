...
function modelMetrics(summary) {
  return {
    ...
    acceptedEvidenceReferences: summary.metrics.counts.evidenceReferences,
...
export function renderReferenceAuditMarkdown(audit) {
  ...
  const lines = [
    ...
    '## Evaluator metrics',
    '',
    '| Run | Provider/model | Thinking | Full pass | QWK | Order | Retest reaction | Diagnostic FP | Accepted evidence refs | Evidence required | Native schema | Unsupported / numeric claims | Tokens | Accepted inference ms | Cost USD |',
    '|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|',
    ...audit.runs.map((run) => `| ${run.runLabel} | ${run.provider}/${run.model} | ${run.thinkingEnabled ? 'on' : 'off'} | ${format(run.fullControlPassRate)} | ${format(run.quadraticWeightedKappa)} | ${format(run.orderConsistency)} | ${format(run.retestReactionAgreement)} | ${format(run.misleadingDiagnosticFalsePositiveRate)} | ${format(run.acceptedEvidenceReferences)} | ${format(run.requiredEvidenceCoverage)} | ${format(run.schemaEnforcementRate)} | ${run.forbiddenClaimViolations} / ${run.numericParameterCandidateViolations} | ${format(run.totalTokens)} | ${format(run.acceptedInferenceMs ?? run.wallClockMs)} | ${format(run.estimatedCostUsd)} |`),
    '',
