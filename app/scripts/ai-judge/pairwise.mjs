...
export function formatPairwiseComparisonPacket({ familyId, edge, caseA, caseB, isSwapped = false }) {
  ...
  return {
    ...
    // Canonical plan identities are deliberately independent of display order so stored
    // traces remain attributable even when A/B is reversed for position-bias control.
    planIdentities: {
      baselinePlanId: caseA.planId ?? null,
      perturbedPlanId: caseB.planId ?? null,
    },
...
export function validateAndNormalizePairwiseResponse(value, { familyId, packet, edge, scale = '0-4' }) {
  ...
  return {
    ...
    // These are generated by the harness rather than echoed by the model, preventing a
    // syntactically valid response from accidentally severing result-to-plan provenance.
    planAId: packet.caseA.planId ?? null,
    planBId: packet.caseB.planId ?? null,
    expectedDirection: edge.expectedDirection,
    expectedMagnitude: edge.expectedMagnitude,
...
export async function executeFamilyPairwiseEvaluations({
  ...
  for (const edge of edges) {
    ...
    pairwiseResults.push({
      ...
      expectedDirection: edge.expectedDirection,
      expectedMagnitude: edge.expectedMagnitude,
      planIdentities: forwardPacket.planIdentities,
      forward: forwardVal,
