...
for (const item of cases.values()) {
  ...
  if (eventDay && !['Rest', 'Mobility/Recovery'].includes(eventDay.session.category)) {
    warnings.push(`${item.input.caseId}: event date ${event.date} also contains planned ${eventDay.session.title}; event-day ownership should be reviewed separately.`);
  }
}

if (warnings.length > 0) {
  console.warn('Plan-judge invariant warnings:');
