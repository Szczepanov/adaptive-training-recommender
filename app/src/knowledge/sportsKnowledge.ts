...
export const SPORTS_KNOWLEDGE_CLAIMS: readonly KnowledgeClaim[] = [
    ...
    },
    {
        ...
        statement: 'For event-free health planning, when Running lacks explicit support (it is not preferred, or it is deprioritized or avoided), the product gives feasible walking/cycling aerobic work a soft preference and limits unnecessary quality-endurance work to one session in a rolling seven-day window. When no endurance, speed/power, or sport-readiness priority is also selected, it additionally withholds Hard Endurance as generic quality filler while keeping preferred Moderate Endurance available. Adverse recovery withholds quality-endurance work until a fresh planning check is available.',
        ...
        limitations: ['This is a product ranking and recovery heuristic, not evidence that walking or cycling is universally superior to running.', 'The Hard Endurance ceiling and one-session/seven-day boundary are conservative calibration values and do not establish universal physiological maximums.', 'Explicit Running support (Running preferred and neither deprioritized nor avoided), an explicit endurance/speed-power/sport-readiness priority, event-directed mode or separate clinical guidance can change the appropriate plan.'],
        reviewedOn: '2026-09-20', version: 2,
    },
    {
        id: KNOWLEDGE_CLAIM_IDS.conditionalHighIntensityPrior,
        statement: 'For an athlete with sufficient and internally consistent recent training history, Evergreen may allocate one high-intensity session as a target and up to two in the weekly plan. The generic prior is withheld during acute adverse recovery, while current pain/injury, illness or red-flag symptoms are reported, and during Post-Event Recovery, but event-proximity Base/Build labels do not replace objective-owned mesocycle intent. When an eligible high-intensity occurrence is actually packed, the weekly packer may reserve capacity by substituting up to one aerobic-volume reservation; the credit follows that role\'s current packed dose but is bounded so at least one full aerobic-volume occurrence remains. If no quality occurrence is packed, the full aerobic requirement is restored.',
        claimType: 'heuristic', maturity: 'heuristic', status: 'active', evidenceCertainty: 'not_applicable', recommendationStrength: 'conditional', safetyImpact: 'moderate',
        ...
        limitations: [
            ...
            'The prior is withheld when recent training evidence is insufficient, limited, conflicting, while current clinical symptoms are reported, or during post-event recovery; symptom withholding is a conservative product rule, not a claim that every symptom contraindicates all intensity.',
    ...
    },
    {
        ...
        statement: 'After a severe adverse-recovery flag, forecast re-entry is deliberately bounded: concordant distress keeps days 1-2 Rest/Mobility-Recovery only, while adverse wearables with a fresh subjective check-in use one recovery-only day; the following day admits non-Strength, non-Moderate/Hard/Race-Specific work at systemicCost <=0.35, and the next two widen that low-intensity pool to <=0.5. A light Race-Specific Endurance taper exception remains available at <=0.45 before A/B events. The unrestricted candidate pool begins on day 5 for fresh-subjective discordance and day 6 for concordant recovery. Separately, after two projected rest days, systemic cost is capped at 0.75. During late severe-recovery re-entry after a Rest day, an Easy Endurance candidate that passes the shared gates is preferred over another Rest day. Forecast recovery-only dates use recover-tier and graduated re-entry dates use modify-tier semantics.',
        claimType: 'heuristic', maturity: 'heuristic', status: 'active', evidenceCertainty: 'not_applicable', recommendationStrength: 'conditional', safetyImpact: 'high',
    ...
    },
    {
        ...
        statement: 'The product internal-response model saturates HRV drop at 15 ms and resting-HR rise at 10 bpm, begins sleep strain below score 75 and Body Battery strain below 50 (saturating at 20), and fuses normalized signals with base systemic weights 0.3 subjective fatigue + 0.25 HRV + 0.25 sleep + 0.2 Body Battery and cardiovascular weights 0.5 resting HR + 0.5 HRV, plus authored soreness/motivation mappings for other dimensions. Conservative non-diluted floors override the weighted sums: systemic 0.60 for subjective fatigue >=8 (0.65 when paired with readiness <=4 or stress >=8, or for readiness <=3 with stress >=8), systemic 0.60 for stress >=9, systemic and cardiovascular 0.80 for concordant HRV <=-10 ms, resting HR >=+5 bpm and Body Battery <=35, and a 0.88 lower-body/impact tissue floor (0.7x on upper body) for soreness >=8.',
        claimType: 'heuristic', maturity: 'heuristic', status: 'active', evidenceCertainty: 'not_applicable', recommendationStrength: 'conditional', safetyImpact: 'moderate',
        ...
        evidence: [{ sourceId: READINESS_SLEEP_HRV_PRODUCT_POLICY_SOURCE, directness: 'direct' }],
        limitations: ['The 15 ms/10 bpm/75/50 thresholds, fusion weights, 8/9/10 subjective cut-points and 0.60/0.65/0.80/0.88 floors are product modeling choices rather than effect sizes from the cited evidence.', 'HRV, resting HR, sleep and subjective symptoms should be treated as complementary signals; none uniquely identifies readiness or maladaptation.', 'Body Battery is a proprietary composite of HRV, stress, sleep and activity, so its 0.2 systemic weight partly re-weights signals already fused separately.', 'This claim records current implementation authority; calibration remains a product-validation problem.'], reviewedOn: '2026-09-24', version: 2,
    },
];
