 ...
 * product-policy claim for the exact scalar. It does NOT mean that an internal product
 * threshold has been scientifically validated. `partial` means a material sub-surface
 * remains unaudited and therefore must keep a research priority.
 */
export const ENGINE_KNOWLEDGE_COVERAGE: readonly EngineKnowledgeCoverageItem[] = [
    ...
    },
    {
        ...
        coverageRationale: 'Recovery literature supports accounting for accumulated stress and individual recovery; the exact 3-day/count-2/+1.0 translation is now explicitly registered as product calibration rather than presented as a scientific cutoff.',
    },
    {
    ...
    },
    {
        ...
        classification: 'product_heuristic', coverage: 'covered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'none',
        codeRefs: ['engine/optimizer.ts:evaluateRecoveryConstraints'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.trainingStressRecoveryBalance, KNOWLEDGE_CLAIM_IDS.anchorSpacing],
        coverageRationale: 'Recovery evidence supports protecting quality under accumulated stress but not a universal one-day rule; the exact previous-day exclusion is now explicitly recorded as conservative product policy.',
    },
    {
        ...
        codeRefs: ['engine/optimizer.ts:buildHistoryFeatureSummary', 'engine/optimizer.ts:evaluateRecoveryConstraints'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.enduranceIntensityDistribution, KNOWLEDGE_CLAIM_IDS.trainingStressRecoveryBalance, KNOWLEDGE_CLAIM_IDS.rollingHardDensityCap],
    ...
    },
    {
        ...
        coverageRationale: 'The scientific recovery boundary and the default 0.6/two-day product fallback now have explicit lineage. Coverage remains partial because catalog-specific recoveryHours/minimumDays values can override the fallback and have not yet been audited workout by workout.',
    },
    {
        ...
        classification: 'product_heuristic', coverage: 'covered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'none',
        codeRefs: ['engine/optimizer.ts:evaluateRecoveryConstraints'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.concurrentStrengthEnduranceContext, KNOWLEDGE_CLAIM_IDS.strengthEnduranceAdjacency],
        coverageRationale: 'Concurrent-training evidence says same-day modalities can be effective and effects are context-dependent; therefore the exact 0-1-day exclusion is deliberately registered as a conservative product quality-protection rule, not a scientific requirement.',
    },
    {
    ...
    },
    {
        ...
        classification: 'product_heuristic', coverage: 'covered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'none',
        codeRefs: ['engine/optimizer.ts:intensityClassForTemplate', 'engine/optimizer.ts:isIntensityClassAdmissible', 'engine/optimizer.ts:normalizeHistory'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.enduranceIntensityDistribution, KNOWLEDGE_CLAIM_IDS.internalLoadIntensityBands],
        coverageRationale: 'The scientific claim provides the distribution-level context; the exact internal 0..1 thresholds are separately registered as product semantics and are not represented as physiological zones.',
    },
    {
    ...
    },
    {
        ...
        coverageRationale: 'Taper duration, maintained intensity and volume reduction are directly researchable and should be handled by the dedicated taper evidence pack; current values have no linked review/guideline claim.',
    },
    {
...
export function validateKnowledgeCoverageInventory(
    ...
    for (const item of items) {
        ...
        if (item.coverage === 'partial' && item.researchPriority === 'none') {
            errors.push(`coverage ${item.id}: partial items must retain a research priority for their unresolved surface`);
        }
        ...
        if (item.coverage === 'partial' && item.decisionImpact === 'high') {
            warnings.push(`coverage ${item.id}: high-impact decision assumption remains only partially covered`);
        }
        if (item.coverage === 'partial' && item.safetyImpact === 'high') {
            warnings.push(`coverage ${item.id}: high-safety assumption remains only partially covered`);
        }
    }
