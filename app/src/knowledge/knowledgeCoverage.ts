...
export interface KnowledgeCoverageValidation {
    valid: boolean;
    errors: string[];
    warnings: string[];
}
 ...
 *
 * Scope rule: include a policy family when changing it can change recommendation mode,
 * eligibility, dose, weekly objectives, ordering, progression, or what physiological
 * evidence is admitted to those decisions. Pure rendering/telemetry, deterministic search
 * bounds, user-authored constraints and shadow-only models are represented only where an
 * explicit not-applicable boundary is useful to prevent them being mistaken for missing
 * sports science.
 *
 * This is intentionally an inventory, not an endorsement of the current values. An
 * `uncovered` record means "the product currently depends on this assumption and we have
 * not yet attached an adequate reviewed knowledge claim", not "the rule is false".
 */
export const ENGINE_KNOWLEDGE_COVERAGE: readonly EngineKnowledgeCoverageItem[] = [
    {
        id: 'evergreen.adult_aerobic_weekly_volume', domain: 'evergreen_dose',
        title: 'Adult aerobic health-volume floor and range',
        ...
        codeRefs: ['engine/evergreenStrategy.ts:aerobicRequirement'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.adultAerobicHealthVolume],
        coverageRationale: 'Directly linked to the active WHO-backed adult aerobic-volume claim.',
    },
    {
        id: 'evergreen.adult_strength_weekly_frequency', domain: 'evergreen_dose',
        title: 'Adult strength health-frequency floor',
        currentRule: 'Evergreen health/balanced planning requires two strength sessions per week.',
        classification: 'scientific_claim', coverage: 'covered', decisionImpact: 'high', safetyImpact: 'low', researchPriority: 'none',
        codeRefs: ['engine/evergreenStrategy.ts:strengthRequirement'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.adultStrengthHealthFrequency],
        coverageRationale: 'Directly linked to the active WHO-backed two-or-more-days strength claim.',
    },
    {
        id: 'evergreen.strength_default_upper_target', domain: 'evergreen_dose',
        title: 'Evergreen strength default upper target',
        ...
        codeRefs: ['engine/evergreenStrategy.ts:strengthRequirement'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.adultStrengthDefaultUpperTarget],
        coverageRationale: 'Explicitly registered as a product heuristic, separate from the WHO >=2-day recommendation.',
    },
    {
        id: 'evergreen.high_intensity_weekly_prior', domain: 'evergreen_dose',
        title: 'Conditional high-intensity weekly prior',
        ...
        codeRefs: ['engine/evergreenStrategy.ts:resolveEvidenceBackedStrategy'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.conditionalHighIntensityPrior],
    ...
    {
        id: 'evergreen.default_weekly_commitment', domain: 'evergreen_dose',
        title: 'Unsaved-profile weekly commitment default',
    ...
    {
        id: 'evergreen.training_history_qualification', domain: 'evergreen_dose',
        title: 'Training-history qualification for performance priors',
    ...
    {
        id: 'packing.legacy_session_spacing_tiebreak', domain: 'session_spacing',
        title: 'Legacy weekly session-spacing tie-break',
    ...
    {
        id: 'readiness.physiological_strain_model', domain: 'readiness_recovery',
        title: 'HRV/RHR/sleep/respiration strain model',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/rules.ts:metricStrain', 'engine/rules.ts:evaluateReadinessAndSafetyEnvelope'], knowledgeRefs: [],
        coverageRationale: 'This live composite can move train -> modify/recover, yet its signal weights, variability floors, cap and chronic multiplier have no registered evidence or explicit product-policy claim.',
    },
    {
        id: 'readiness.subjective_mode_thresholds', domain: 'readiness_recovery',
        title: 'Subjective readiness mode thresholds',
        ...
        coverageRationale: 'These thresholds can directly suppress training and are presented as safety/readiness authority without claim-level provenance.',
    },
    {
        id: 'readiness.absolute_device_floors', domain: 'readiness_recovery',
        title: 'Absolute sleep-score and Body Battery floors',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/rules.ts:evaluateReadinessAndSafetyEnvelope', 'engine/rules.ts:evaluateEnvelopes'], knowledgeRefs: [],
        coverageRationale: 'Vendor-derived scores are used as direct policy thresholds; neither the threshold values nor their translation to training restriction is registered.',
    },
    {
        id: 'readiness.acute_biometric_floors', domain: 'readiness_recovery',
        title: 'Acute HRV/RHR hard modify floors',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/rules.ts:evaluateReadinessAndSafetyEnvelope'], knowledgeRefs: [],
        coverageRationale: 'Absolute within-athlete deviation cutoffs are decision authority and need evidence about reliability, meaningful change and appropriate action.',
    },
    {
        id: 'readiness.recent_hard_session_penalty', domain: 'readiness_recovery',
        title: 'Recent hard-session readiness penalty',
        currentRule: 'Two or more hard sessions in the prior three days adds 1.0 to readiness strain.',
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/rules.ts:evaluateReadinessAndSafetyEnvelope'], knowledgeRefs: [],
        coverageRationale: 'Conceptually related to the Evergreen weekly high-intensity cap, but that claim does not justify a 3-day window, threshold of two, or +1.0 readiness penalty.',
    },
    {
        id: 'readiness.mode_score_thresholds', domain: 'readiness_recovery',
        title: 'Composite strain train/modify/recover thresholds',
        currentRule: 'Composite objective strain >=1.0 modifies and >=2.2 recovers; conservative mode adds 0.4 before those thresholds.',
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'high', researchPriority: 'p0',
        codeRefs: ['engine/rules.ts:evaluateReadinessAndSafetyEnvelope'], knowledgeRefs: [],
        coverageRationale: 'These are the final live cut-points over several biometric heuristics and therefore need explicit validation/provenance even if retained as product policy.',
    },
    {
        id: 'readiness.post_recover_buffer', domain: 'readiness_recovery',
        title: 'Post-recover one-day buffer',
    ...
    {
        id: 'readiness.already_trained_fail_closed', domain: 'readiness_recovery',
        title: 'Already-trained same-day override',
    ...
    {
        id: 'readiness.plan_tier_cost_ceilings', domain: 'readiness_recovery',
        title: 'Readiness plan-tier systemic cost ceilings',
        ...
        coverageRationale: 'The mapping from readiness state to allowed catalog cost is product-calibrated and has no explicit knowledge claim.',
    },
    {
        id: 'fatigue.dimension_half_lives', domain: 'fatigue_load',
        title: 'Dimensional fatigue decay half-lives',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/fatigue.ts:DECAY_HALF_LIVES_HOURS', 'engine/fatigue.ts:decayFatigue'], knowledgeRefs: [],
        coverageRationale: 'These latent recovery kinetics feed future candidate costs and spacing but are not backed by a registered model/evidence claim.',
    },
    {
        id: 'fatigue.internal_response_model', domain: 'fatigue_load',
        title: 'Internal response strain normalization and fusion weights',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/fatigue.ts:computeInternalResponseStrain'], knowledgeRefs: [],
        coverageRationale: 'A second live readiness/fatigue model with material thresholds and weights currently has no claim-level provenance or calibration record.',
    },
    {
        id: 'fatigue.ambient_step_surge', domain: 'fatigue_load',
        title: 'Ambient step-surge tissue-strain heuristic',
    ...
    {
        id: 'fatigue.max_fusion_policy', domain: 'fatigue_load',
        title: 'External/internal fatigue fusion policy',
    ...
    {
        id: 'safety.minimum_checkin_gate', domain: 'injury_safety',
        title: 'Minimum safety check-in fail-closed gate',
    ...
    {
        id: 'injury.tissue_response_severity', domain: 'injury_safety',
        title: 'Tissue-response to restriction severity mapping',
    ...
    {
        id: 'injury.region_restriction_mapping', domain: 'injury_safety',
        title: 'Body-region restriction mapping',
    ...
    {
        id: 'injury.pain_envelope_mapping', domain: 'injury_safety',
        title: 'Generic current-day pain envelope',
    ...
    {
        id: 'spacing.anchor_next_day', domain: 'session_spacing',
        title: 'Anchor/quality next-day spacing gate',
        currentRule: 'A candidate anchor is rejected when an anchor occurred one day earlier.',
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/optimizer.ts:evaluateRecoveryConstraints'], knowledgeRefs: [],
        coverageRationale: 'A hard 1-day quality-spacing gate directly removes sessions and should be tied to evidence about recovery/quality-session sequencing or explicitly retained as product conservatism.',
    },
    {
        id: 'spacing.rolling_hard_cap', domain: 'session_spacing',
        title: 'Rolling hard-session density cap',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'high', researchPriority: 'p0',
        codeRefs: ['engine/optimizer.ts:buildHistoryFeatureSummary', 'engine/optimizer.ts:evaluateRecoveryConstraints'], knowledgeRefs: [],
        coverageRationale: 'This live density gate is distinct from the Evergreen weekly high-intensity prior: it uses catalog systemicCost and a rolling six-day window and currently has no supporting claim.',
    },
    {
        id: 'spacing.hard_lower_body_recovery', domain: 'session_spacing',
        title: 'Hard lower-body minimum spacing and workout recovery hours',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'high', researchPriority: 'p0',
        codeRefs: ['engine/optimizer.ts:evaluateRecoveryConstraints', 'engine/planningCandidate.ts:resolveMinimumDaysAfterHardLowerBody', 'engine/planningCandidate.ts:resolveRecoveryHoursForTemplate'], knowledgeRefs: [],
        coverageRationale: 'Both the 0.6/2-day fallback and catalog-authored recovery metadata act as training authority without a claim-level basis or validation contract.',
    },
    {
        id: 'spacing.strength_key_cycling_adjacency', domain: 'session_spacing',
        title: 'Heavy lower-body strength vs key-cycling adjacency',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p0',
        codeRefs: ['engine/optimizer.ts:evaluateRecoveryConstraints'], knowledgeRefs: [],
        coverageRationale: 'Concurrent-training interference/recovery is a researchable domain, but the current exact adjacency window is not linked to reviewed evidence.',
    },
    {
        id: 'spacing.pre_event_restrictions', domain: 'session_spacing',
        title: 'Pre-event strength, hard and exhaustive session restrictions',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p1',
        codeRefs: ['engine/optimizer.ts:evaluateRecoveryConstraints'], knowledgeRefs: [],
        coverageRationale: 'These are taper/freshness rules with directly researchable timing questions but no registered evidence.',
    },
    {
        id: 'optimizer.intensity_class_thresholds', domain: 'optimizer_scoring',
        title: 'Catalog systemic-cost intensity classification',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p1',
        codeRefs: ['engine/optimizer.ts:intensityClassForTemplate', 'engine/optimizer.ts:isIntensityClassAdmissible', 'engine/optimizer.ts:normalizeHistory'], knowledgeRefs: [],
        coverageRationale: 'The internal 0..1 cost/intensity scale is product-defined; cut-points need explicit calibration provenance rather than appearing physiological.',
    },
    {
        id: 'optimizer.fatigue_cost_weights', domain: 'optimizer_scoring',
        title: 'Fatigue cost-penalty weights',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p1',
        codeRefs: ['engine/optimizer.ts:calculateFatigueCostPenalty'], knowledgeRefs: [],
        coverageRationale: 'These weights materially reorder accepted sessions but have no registered calibration or product-policy claim.',
    },
    {
        id: 'optimizer.stimulus_benefit_weights', domain: 'optimizer_scoring',
        title: 'Objective stimulus-benefit weights',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'low', researchPriority: 'p2',
        codeRefs: ['engine/optimizer.ts:calculateStimulusBenefit'], knowledgeRefs: [],
        coverageRationale: 'Utility calibration determines preference among otherwise feasible sessions but is not an evidence claim about adaptation magnitude.',
    },
    {
        id: 'optimizer.event_priority_multipliers', domain: 'optimizer_scoring',
        title: 'Event-priority and horizon ranking multipliers',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'low', researchPriority: 'p2',
        codeRefs: ['engine/optimizer.ts:rankCandidates'], knowledgeRefs: [],
        coverageRationale: 'Explicit product utility shaping with no claim-level provenance; research may inform horizon/variation but exact multipliers require calibration.',
    },
    {
        id: 'optimizer.recovery_streak_heuristics', domain: 'optimizer_scoring',
        title: 'Recovery alternation and training-streak heuristics',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'moderate', researchPriority: 'p1',
        codeRefs: ['engine/optimizer.ts:buildHistoryFeatureSummary', 'engine/optimizer.ts:rankCandidates'], knowledgeRefs: [],
        coverageRationale: 'Recovery-day/streak policy can substantially alter ranking and is currently calibrated only in code.',
    },
    {
        id: 'periodization.phase_boundaries_scales', domain: 'periodization_taper',
        title: 'Base/Build/Specificity phase boundaries and dose scales',
        ...
        coverageRationale: 'Classic periodization concepts may be evidence-informed, but these exact phase boundaries, blends and scalars are unregistered product policy.',
    },
    {
        id: 'periodization.taper_windows_volume', domain: 'periodization_taper',
        title: 'Event taper windows and volume reduction',
        ...
        codeRefs: ['engine/taperPolicy.ts:resolveEventTaper', 'engine/periodization.ts:evaluatePeriodizationPhase'], knowledgeRefs: [],
        coverageRationale: 'Taper duration, maintained intensity and volume reduction are directly researchable and should be the first periodization evidence pack; current values have no linked review/guideline claim.',
    },
    {
        id: 'periodization.objective_thresholds', domain: 'periodization_taper',
        title: 'Demand-to-weekly-objective thresholds',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'p1',
        codeRefs: ['engine/periodization.ts:objectivesFromDemand', 'engine/microcycle.ts:generateWeeklyObjectives'], knowledgeRefs: [],
        coverageRationale: 'The normalized demand/stimulus scale is product-defined; threshold meanings and exposure counts are not registered or empirically calibrated.',
    },
    {
        id: 'periodization.taper_sharpening_targets', domain: 'periodization_taper',
        title: 'Race-week sharpening and strength-primer targets',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'moderate', researchPriority: 'p1',
        codeRefs: ['engine/periodization.ts:TAPER_SHARPENING_TARGET_STIMULUS', 'engine/periodization.ts:TAPER_STRENGTH_TARGET_STIMULUS'], knowledgeRefs: [],
        coverageRationale: 'The concepts align with freshness/maintenance goals, but the internal target values have no claim-level provenance.',
    },
    {
        id: 'periodization.multi_event_contribution', domain: 'periodization_taper',
        title: 'Multi-event contribution window and taper authority',
        ...
        codeRefs: ['engine/periodization.ts:resolveMultiEventObjectives'], knowledgeRefs: [],
        coverageRationale: 'Mostly conflict-resolution product policy, but its 35/14/5-day timing assumptions can alter training and should be documented/calibrated.',
    },
    {
        id: 'event.demand_presets', domain: 'event_demand',
        title: 'Sport/event demand profiles',
        ...
        codeRefs: ['engine/eventPresets.ts:EVENT_PRESETS'], knowledgeRefs: [],
        coverageRationale: 'These profiles encode sport-specific physiological assumptions and directly shape objectives/periodization, but currently have no evidence sources or product-policy claims.',
    },
    {
        id: 'stimulus.objective_credit_confidence', domain: 'stimulus_credit',
        title: 'Stimulus-evidence confidence discount',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'moderate', researchPriority: 'p2',
        codeRefs: ['engine/stimulus.ts:CONFIDENCE_CREDIT_WEIGHT', 'engine/stimulus.ts:deriveObjectiveCreditFromProfile'], knowledgeRefs: [],
        coverageRationale: 'This is evidence-trust calibration rather than sports physiology, but it affects whether future training objectives remain unresolved and needs an explicit product-policy basis.',
    },
    {
        id: 'stimulus.legacy_keyword_credit', domain: 'stimulus_credit',
        title: 'Legacy keyword compatibility credit',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'low', safetyImpact: 'low', researchPriority: 'p3',
        codeRefs: ['engine/microcycle.ts:LEGACY_KEYWORD_COMPATIBILITY_CREDIT', 'engine/microcycle.ts:updateMicrocycleProgress'], knowledgeRefs: [],
        coverageRationale: 'Compatibility confidence is deliberately conservative but currently lacks an explicit product-policy claim.',
    },
    {
        id: 'stimulus.race_specific_credit_formula', domain: 'stimulus_credit',
        title: 'Race-specific objective credit formula',
        ...
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'low', researchPriority: 'p2',
        codeRefs: ['engine/stimulus.ts:deriveObjectiveCreditFromProfile'], knowledgeRefs: [],
        coverageRationale: 'A product mapping from authored stimulus semantics to objective completion; its equal blend and max operators are not evidence-backed adaptation estimates.',
    },
    {
        id: 'stimulus.coverage_threshold', domain: 'stimulus_credit',
        title: 'Stimulus coverage qualification threshold',
        currentRule: 'Generic stimulus coverage uses a 0.6 qualification threshold where consumed by compatibility/planning paths.',
        classification: 'product_heuristic', coverage: 'uncovered', decisionImpact: 'moderate', safetyImpact: 'low', researchPriority: 'p2',
        codeRefs: ['engine/microcycle.ts:STIMULUS_CREDIT_COVERAGE_THRESHOLD'], knowledgeRefs: [],
        coverageRationale: 'The 0.6 threshold is an internal semantic cutoff with no registered calibration.',
    },
    {
        id: 'planning.user_capacity_authority', domain: 'planning_capacity',
        title: 'User-authored time/equipment/environment capacity',
        ...
        codeRefs: ['engine/eligibility.ts:evaluateTemplateEligibility', 'engine/trainingCapacity.ts:resolveTrainingCapacity'], knowledgeRefs: [],
        coverageRationale: 'This applies explicit user/availability constraints and deterministic feasibility arithmetic; it is not a sports-knowledge claim.',
    },
    {
        id: 'planning.search_budgets', domain: 'planning_capacity',
        title: 'Deterministic weekly allocation search budgets',
        ...
        codeRefs: ['engine/weeklyAllocation.ts:WEEKLY_ALLOCATION_SEARCH_BUDGET'], knowledgeRefs: [],
        coverageRationale: 'These are deterministic computational-resource bounds; scientific provenance would be category error. Budget exhaustion is surfaced rather than treated as physiological infeasibility.',
    },
    {
        id: 'data_trust.identity_gated_source_fail_closed', domain: 'data_trust',
        title: 'Identity-gated health-source eligibility',
        ...
        classification: 'safety_invariant', coverage: 'not_applicable', decisionImpact: 'high', safetyImpact: 'high', researchPriority: 'none',
        codeRefs: ['engine/identityEligibility.ts:selectEligibleHealthObservationBundles'], knowledgeRefs: [],
        coverageRationale: 'This is provenance/identity fail-closed behavior protecting physiological inputs, not a sports-science claim. The shadow classifier thresholds that may create projections require separate activation evidence before becoming authority.',
    },
];
...
export function validateKnowledgeCoverageInventory(
    ...
    for (const item of items) {
        ...
        if (item.coverage === 'covered' && item.knowledgeRefs.length === 0) {
            errors.push(`coverage ${item.id}: covered items require at least one knowledgeRef`);
        }
        if (item.coverage === 'partial' && item.knowledgeRefs.length === 0) {
            errors.push(`coverage ${item.id}: partial items require at least one existing knowledgeRef`);
        }
        if ((item.coverage === 'uncovered' || item.coverage === 'not_applicable') && item.knowledgeRefs.length > 0) {
            errors.push(`coverage ${item.id}: ${item.coverage} items must not claim knowledgeRefs`);
        }
        if (item.coverage === 'not_applicable'
            && item.classification !== 'implementation_constant'
            && item.classification !== 'safety_invariant') {
            errors.push(`coverage ${item.id}: not_applicable is reserved for implementation constants and non-scientific safety invariants`);
        }
        if (item.coverage === 'covered' && item.researchPriority !== 'none') {
            errors.push(`coverage ${item.id}: covered items must not retain a research priority`);
        }
        if (item.coverage === 'uncovered' && item.researchPriority === 'none') {
            errors.push(`coverage ${item.id}: uncovered items require a research priority`);
        }
        if (item.coverage === 'not_applicable' && item.researchPriority !== 'none') {
            errors.push(`coverage ${item.id}: not_applicable items must not enter the research backlog`);
        }

        for (const claimId of item.knowledgeRefs) {
            try {
                getActiveKnowledgeClaim(claimId);
            } catch (error) {
                errors.push(`coverage ${item.id}: knowledgeRef ${claimId} is not an active known claim (${error instanceof Error ? error.message : String(error)})`);
            }
        }

        if (item.coverage === 'uncovered' && item.decisionImpact === 'high') {
            warnings.push(`coverage ${item.id}: high-impact decision assumption has no knowledge claim`);
        }
        if (item.coverage === 'uncovered' && item.safetyImpact === 'high') {
            warnings.push(`coverage ${item.id}: high-safety assumption has no knowledge claim`);
        }
    }
...
function emptyCoverageCounts(): Record<KnowledgeCoverageState, number> {
    return { covered: 0, partial: 0, uncovered: 0, not_applicable: 0 };
}

function emptyClassificationCounts(): Record<KnowledgeAssumptionClassification, number> {
    return { scientific_claim: 0, product_heuristic: 0, athlete_specific_rule: 0, safety_invariant: 0, implementation_constant: 0 };
}

function emptyPriorityCounts(): Record<KnowledgeResearchPriority, number> {
    return { p0: 0, p1: 0, p2: 0, p3: 0, none: 0 };
}
...
/**
 * Reviewed non-authority surfaces. Keeping this boundary next to the inventory prevents a
 * shadow model or observability score from being counted as a live knowledge gap merely
 * because it contains provisional thresholds.
 */
