...
export const ENGINE_KNOWLEDGE_COVERAGE: readonly EngineKnowledgeCoverageItem[] = [
    ...
    },
    {
        ...
        currentRule: 'After a severe adverse-recovery flag, projected forecast days 4-5 stay capped to Rest/Mobility-Recovery or non-Strength candidates at systemicCost <=0.35; the unrestricted candidate pool is reached only from day 6 onward (days 1-3 keep their existing rest/modify/0.65-excluding-Hard-and-Race-Specific tiers).',
        classification: 'product_heuristic', coverage: 'covered', decisionImpact: 'high', safetyImpact: 'high', researchPriority: 'none',
        codeRefs: ['engine/planner.ts:RECOVERY_REENTRY_MAX_SYSTEMIC_COST'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.trainingStressRecoveryBalance, KNOWLEDGE_CLAIM_IDS.severeAdverseRecoveryReentry],
        coverageRationale: 'Registered as an explicit product-policy claim (`policy.load_recovery.severe_adverse_recovery_reentry_v1`) with alignment testing. Recovery literature supports accounting for accumulated stress before resuming quality work; the exact 5-day window and 0.35 systemic-cost ceiling are product calibration, and a forecast has no live future readiness signal to check against, so this graduated ramp is the closest implementable equivalent to a fresh-check requirement.',
    },
    {
    ...
    },
    {
        ...
        coverageRationale: 'Registered as an explicit product-policy claim (`policy.optimizer.event_priority_multipliers_v1`) with alignment testing. Multipliers shape candidate utility toward target event deadlines and priorities while preserving the strength-meet and unrelated-work exceptions in production.',
    },
    {
