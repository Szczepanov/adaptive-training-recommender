...
export const ENGINE_KNOWLEDGE_COVERAGE: readonly EngineKnowledgeCoverageItem[] = [
    ...
    },
    {
        ...
        currentRule: 'For A/B/C cycling/running/triathlon events: strength is blocked 1-3 days pre-race; hard work is blocked 1-2 days; generic Moderate/Hard Endurance is also blocked at D-3 while light Race-Specific Endurance may remain available; exhaustive work (systemicCost >=0.75 or VO2 title) is blocked 3-7 days for A/B, while C-priority events without an active authored taper train through D-3 and still obey D-1/D-2 gates. An active athlete-authored taper restores the exhaustive and repeat-session restrictions for C. Across the full resolved taper window (same cycling/running/triathlon categories, not strength_meet): a second-or-later or non-light (systemicCost >0.35) strength candidate is excluded as nonessential, and a Moderate/Hard Endurance candidate is excluded within 3 days of a prior one.',
