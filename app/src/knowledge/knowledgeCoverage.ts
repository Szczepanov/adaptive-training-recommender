...
export const ENGINE_KNOWLEDGE_COVERAGE: readonly EngineKnowledgeCoverageItem[] = [
    ...
    {
        id: 'optimizer.preferred_modality_fallback', domain: 'optimizer_scoring', title: 'Preferred-modality fallback and catalog strength spacing',
        currentRule: 'All automatic Field catalog templates require explicit Field preference. When eligible preferred training exists, unpreferred non-recovery, non-event-directed candidates that do not satisfy an unresolved weekly objective retain eligibility with 0.25x benefit and utility. An explicit current-day preference wins within the same coverage/recovery/objective-benefit tier through the final variety tie-break. Automatic catalog strength recommendations avoid adjacent athlete-local dates across all strength categories, while shared authored-plan critique retains its established spacing rules.',
        classification: 'product_heuristic', coverage: 'covered', decisionImpact: 'high', safetyImpact: 'moderate', researchPriority: 'none',
        codeRefs: ['engine/optimizer.ts:rankCandidates', 'engine/optimizer.ts:hasAdjacentStrengthExposure'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.preferredModalityFallbackPolicy],
        coverageRationale: 'Issue #736 registers these candidate-selection rules as product policy, with deterministic alignment tests. Two athlete-local dates express scheduling spacing, not an elapsed-hour medical claim.',
    },
    {
