...
export const ENGINE_KNOWLEDGE_COVERAGE: readonly EngineKnowledgeCoverageItem[] = [
    ...
    },
    {
        ...
        currentRule: 'Event matching is modality-based: cycling->Cycling, running->Running, strength meet->Strength, triathlon->Cycling/Running. Matching A/B/C events multiply benefit by 1.40/1.25/1.00; for strength meets the boost requires an unresolved objective match. A second B or C race-specific session in six days is multiplied by 0.35; race-specific work >21 days out by 0.50, except for cycling durability events (cycling_event with aerobicEndurance >=0.8 and fatigueResistance >=0.8 and repeatedSurges <0.6) where the 0.50 multiplier is skipped. Non-matching work is multiplied by 0.20 only with unresolved objectives when it is neither preferred nor itself objective-matching. C events retain the no-default-taper train-through policy while remaining bounded against unconstrained race-specific volume.',
        classification: 'product_heuristic', coverage: 'covered', decisionImpact: 'moderate', safetyImpact: 'low', researchPriority: 'none',
        codeRefs: ['engine/optimizer.ts:rankCandidates'],
        knowledgeRefs: [KNOWLEDGE_CLAIM_IDS.eventPriorityMultipliersPolicy],
        coverageRationale: 'Registered as an explicit product-policy claim (`policy.optimizer.event_priority_multipliers_v2`) with alignment testing. Multipliers shape candidate utility toward target event deadlines and priorities while preserving the strength-meet, C no-default-taper, and unrelated-work exceptions in production; all endurance priorities still receive the precise pre-event session restrictions.',
    },
    {
