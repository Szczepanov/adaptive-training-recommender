...
export const STRENGTH_CONCURRENT_CLAIMS: readonly KnowledgeClaim[] = [
    {
        ...
        statement: 'Cycling build strength-support policy v1: when an athlete whose durable training intent explicitly includes strength_muscle is in an event-directed cycling plan, the build block keeps the rest of the evergreen weekly strength floor (the WHO-backed two-session floor minus the one authored primary-strength role, i.e. one) as exact compact_strength support roles. Support roles earn no second physiological strength objective credit and do not inflate primary_strength coverage. The weekly allocator places them only after the unchanged primary-role allocation, on dates it left free, never on a date nominated to a primary role or on the weekly quality/event-specific anchor dates, and never in a way that invalidates a later primary reservation; otherwise it reports a typed miss. Same-day ranking treats an unmet support minimum as deferred support rather than an urgent primary role. Peak, taper and race blocks do not carry them. Readiness, fatigue, spacing, tissue, rolling-load and time gates remain authoritative.',
        claimType: 'heuristic',
        maturity: 'heuristic',
        status: 'active',
