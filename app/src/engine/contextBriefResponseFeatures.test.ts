...
const intervalRide = (powers: [number, number, number]) => ride({
    ...
    laps: [
        ...
        lap(6, 11, powers[2], 155), lap(7, 13, 130, 128),
    ],
});
...
describe('interval repetition (#814)', () => {
    ...
    it('withholds interval HR when the HR measurement is unreliable', () => {
        const feature = deriveIntervalRepetition({
            ...intervalRide([229, 225, 237]),
            hrMeasurement: { measurementConfidence: 'unreliable', signalQuality: 'poor' } as HrMeasurement,
        });
...
describe('cardiac drift / decoupling (#814)', () => {
    it('computes decoupling for a steady ride', () => {
    ...
    it('refuses a variable unstructured ride', () => {
        ...
        const text = renderKeySessionSummaries(
            deriveKeySessionSummaries([variable], { history: [variable], historyStart: '2026-08-22', checkins: [], asOfDate: '2026-09-20' }),
            { history: [variable], historyStart: '2026-08-22', checkins: [], asOfDate: '2026-09-20' },
        );
...
describe('aerobic-efficiency comparison (#814)', () => {
    ...
    it('gives high confidence only for the same device structured workout with known-good HR', () => {
        const hr = { measurementConfidence: 'high', signalQuality: 'clean', summaryCompatibility: 'verified_same_effective_trace' } as HrMeasurement;
        const feature = deriveEfficiencyComparison(
            { ...current, fitWorkoutFingerprint: 'fw1', hrMeasurement: hr },
            [steady('prior', '2026-09-10', { fitWorkoutFingerprint: 'fw1', hrMeasurement: hr })],
        );
        expect(feature.state === 'available' && feature.confidence).toBe('high');
    });
...
describe('next-day response (#814)', () => {
    ...
    it('links the next-morning check-in and labels it observational', () => {
        const context = { history: [session], historyStart: '2026-08-22', checkins: [checkin('2026-09-18', 3, 4), checkin('2026-09-19', 6, 5)], asOfDate: '2026-09-20' };
    ...
    it('distinguishes an unreadable check-in history from a missing check-in', () => {
        ...
        expect(deriveNextDayResponse(session, [], [], '2026-09-20')).toEqual({ state: 'insufficient_evidence', reason: 'no next-morning check-in recorded' });
        expect(deriveNextDayResponse(session, [], [], '2026-09-18')).toEqual({ state: 'insufficient_evidence', reason: 'next morning not yet reached' });
    });
});
...
describe('planning vs diagnostic export (#814)', () => {
    ...
    const context = { history: [session], historyStart: '2026-08-22', checkins: [], asOfDate: '2026-09-20' };
