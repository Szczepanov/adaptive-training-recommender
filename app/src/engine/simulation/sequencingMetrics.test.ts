...
describe('computeSequencingDiagnostics (issue #458)', () => {
    describe('residual-fatigue collision (§4.1)', () => {
        it('is scale-invariant and exposes cost magnitude so small and large sessions remain distinguishable', () => {
            const t = trace({
            ...
            expect(out.residualFatigueCollision.perDay[0].costMagnitude).toBeCloseTo(0.01, 6);
        });
        ...
        it('reports zero (not NaN) for a genuinely zero-cost day', () => {
            ...
            expect(out.residualFatigueCollision.perDay[0].costMagnitude).toBe(0);
    ...
    describe('adjacent cost-vector overlap (§4.2)', () => {
        ...
        it('normalizes overlap with weighted Jaccard/Ruzicka similarity after fatigue decay', () => {
            const a = trace({ date: '2026-01-01', weekIndex: 0, category: 'Hard Endurance', cost: { lowerBody: 0.8 } });
            const b = trace({ date: '2026-01-02', weekIndex: 0, category: 'Hard Endurance', cost: { lowerBody: 0.8 } });
            const out = computeSequencingDiagnostics([a, b]);
            const expected = Math.pow(0.5, 24 / 48); // lower-body half-life = 48 h
            expect(out.adjacentCostOverlap.perPair[0].overlap).toBeCloseTo(expected, 6);
            expect(out.adjacentCostOverlap.perPair[0].overlap).toBeGreaterThanOrEqual(0);
            expect(out.adjacentCostOverlap.perPair[0].overlap).toBeLessThanOrEqual(1);
        });
        ...
        it('reports high dimension-specific overlap for two adjacent high-lower-body-cost days', () => {
    ...
    describe('hard-day concentration & recovery placement (§4.4)', () => {
        ...
        it('preserves recovery-placement attribution across a week boundary', () => {
            const hard = trace({
                date: '2026-01-07', weekIndex: 0, category: 'Hard Endurance',
                cost: { systemic: 0.9 }, combinedFatigue: { systemic: 0.9 },
            });
            const rest = trace({ date: '2026-01-08', weekIndex: 1, category: 'Rest' });
            const out = computeSequencingDiagnostics([hard, rest]);
            expect(out.hardDayConcentration.weekly[0].recoveryAfterHighCollisionCount).toBe(0);
            expect(out.hardDayConcentration.weekly[1].recoveryAfterHighCollisionCount).toBe(1);
        });
    ...
    describe('opportunity-cost / sequence-regret (§4.5)', () => {
        ...
        it('aggregates a tier-blocked day and attributes it to the correct tier', () => {
            const t = trace({
                date: '2026-01-01', weekIndex: 0, category: 'Easy Endurance',
            ...
            const out = computeSequencingDiagnostics([t]);
            expect(out.opportunityCost.utilityWinnerDifferentCount).toBe(1);
            ...
            expect(out.opportunityCost.perDay[0]).toMatchObject({
                date: '2026-01-01', selectedTemplateId: 'selected', bestUtilityTemplateId: 'blocked',
                selectedVsBestUtilityGap: 2.5, tierBlocked: true,
                blockedByTier: { coverage: true, recovery: false, benefit: false },
            });
        });

        it('separates a same-tier utility difference from ordinal blocking', () => {
            const t = trace({
                date: '2026-01-01', weekIndex: 0, category: 'Easy Endurance',
                rankingAudit: rankingAudit({
                    selectedTemplateId: 'selected', bestUtilityTemplateId: 'higher-utility-same-tier',
                    selectedVsBestUtilityGap: 0.04,
                }),
            });
            const out = computeSequencingDiagnostics([t]);
            expect(out.opportunityCost.utilityWinnerDifferentCount).toBe(1);
            expect(out.opportunityCost.utilityWinnerBlockedCount).toBe(0);
            expect(out.opportunityCost.utilityWinnerDifferentWithoutTierBlockCount).toBe(1);
            expect(out.opportunityCost.meanBlockedUtilityGap).toBe(0);
            expect(out.opportunityCost.maxBlockedUtilityGap).toBe(0);
            expect(out.opportunityCost.blockedByTier).toEqual({ coverage: 0, recovery: 0, benefit: 0 });
            expect(out.opportunityCost.perDay[0].tierBlocked).toBe(false);
        });
