...
function trainingSettings(overrides: Partial<TrainingSettings['equipment']> = {}, defaults: Partial<TrainingSettings['defaults']> = {}): TrainingSettings {
    return {
        userId: 'sim-user',
        schemaVersion: 2,
        equipment: { free_weights: true, cable_machine: false, treadmill: false, indoor_bike: false, pullup_bar: false, outdoor_bike: false, swim_access: false, ...overrides },
...
export const SCENARIOS: AthleteScenario[] = [
    ...
    {
        id: 'triathlon_novice_eighth_A',
        label: 'Novice triathlon A-event (1/8 distance, 40 days out)',
        description: 'No seeded training history: this is a first short-distance triathlon athlete with explicit pool and bicycle access. The engine must make all three disciplines reachable without inferring advanced capacity.',
        context: context({ outdoor_bike: true, swim_access: true }, ['Swimming', 'Cycling', 'Running']),
        event: eventOn('e-tri-novice', 40, 'triathlon', 'eighth_im', 'A'),
        startDate: START_DATE,
        weeks: 4,
        readinessForWeek: () => stableReadiness({ readiness: 7, fatigue: 3, soreness: 2, motivation: 8 }),
    },
    {
        id: 'triathlon_advanced_half_iron_A',
        label: 'Advanced triathlon A-event (70.3 distance, 40 days out)',
        description: 'Long-course triathlon control with declared pool and bicycle access plus a generous but bounded schedule. It exercises three-discipline coverage without claiming a specialist distance-progression model.',
        context: context({ indoor_bike: true, free_weights: true, treadmill: true, outdoor_bike: true, swim_access: true }, ['Swimming', 'Cycling', 'Running'], [], { weekdayMaxMinutes: 90, weekendMaxMinutes: 120 }),
        event: eventOn('e-tri-advanced', 40, 'triathlon', 'half_iron', 'A'),
        startDate: START_DATE,
        weeks: 4,
        readinessForWeek: () => stableReadiness({ readiness: 8, fatigue: 2, soreness: 2, motivation: 8, timeAvailable: 90 }),
    },
    {
        id: 'triathlon_advanced_half_iron_taper_A',
        label: 'Advanced triathlon A-event (70.3 distance, final 14-day taper)',
        description: 'A race-day boundary control for the advanced long-course athlete. The two-week horizon ends before the event, so taper behavior is assessed without manufacturing post-event training.',
        context: context({ indoor_bike: true, free_weights: true, treadmill: true, outdoor_bike: true, swim_access: true }, ['Swimming', 'Cycling', 'Running'], [], { weekdayMaxMinutes: 90, weekendMaxMinutes: 120 }),
        event: eventOn('e-tri-advanced-taper', 14, 'triathlon', 'half_iron', 'A'),
        startDate: START_DATE,
        weeks: 2,
        readinessForWeek: () => stableReadiness({ readiness: 8, fatigue: 2, soreness: 2, motivation: 8, timeAvailable: 90 }),
    },
