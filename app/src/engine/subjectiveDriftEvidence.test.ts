...
function baseline(gap: number): SubjectiveBaseline {
    ...
    return {
        estimatorId: 'test-evidence-v1', historyThroughDateExclusive: '2026-08-16', recentRecordedDays: 7, longRecordedDays: 28, lastObservationDate: '2026-08-15',
        metrics: { readiness: higherBetter(8), sleepQuality: higherBetter(8), motivation: higherBetter(8), fatigue: lowerBetter(2), soreness: lowerBetter(2), mentalStress: lowerBetter(2) },
    };
}
...
describe('simulation buildSubjectiveDriftDecisionEvidence', () => {
    ...
    it('clamps a negative metric weight to zero through the canonical helper', () => {
        const e = buildSubjectiveDriftDecisionEvidence({ subjective, objective, subjectiveBaseline: baseline(0.5) }, context, undefined, undefined, { ...REFERENCE_SUBJECTIVE_DRIFT_WEIGHTS, readiness: -100 })!;
...
describe('compactSubjectiveDriftAudit', () => {
    it('drops simulation-only counterfactual fields', () => {
        const audit = compactSubjectiveDriftAudit(buildSubjectiveDriftDecisionEvidence({ subjective, objective, subjectiveBaseline: baseline(0.25) }, context));
        expect(audit).toMatchObject({ estimatorId: 'test-evidence-v1', historyThroughDateExclusive: '2026-08-16', decisionRelevant: true });
