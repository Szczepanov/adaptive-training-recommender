...
import { buildSubjectiveDriftDecisionEvidence, compactSubjectiveDriftAudit } from './subjectiveDriftEvidence';
...
function baseline(gap: number): SubjectiveBaseline {
    ...
    return {
        estimatorId: 'test-evidence-v1',
        historyThroughDateExclusive: '2026-08-16',
        recentRecordedDays: 7,
        longRecordedDays: 28,
        lastObservationDate: '2026-08-15',
        metrics: {
            readiness: higherBetter(8),
            sleepQuality: higherBetter(8),
            motivation: higherBetter(8),
            fatigue: lowerBetter(2),
            soreness: lowerBetter(2),
            mentalStress: lowerBetter(2),
        },
    };
}

describe('buildSubjectiveDriftDecisionEvidence', () => {
    ...
    it('reconciles its per-metric components to the canonical total and records the true counterfactual', () => {
        const evidence = buildSubjectiveDriftDecisionEvidence(
            { subjective, objective, subjectiveBaseline: baseline(0.25) },
            context,
        );
        expect(evidence).not.toBeNull();
        const e = evidence!;
        const componentSum = Object.values(e.perMetricContributions).reduce((a, b) => a + b, 0);
        expect(componentSum).toBeCloseTo(e.contribution, 10);
    ...
    it('uses one-hot canonical weights rather than allowing a negative metric weight to subtract', () => {
        const evidence = buildSubjectiveDriftDecisionEvidence(
            { subjective, objective, subjectiveBaseline: baseline(0.5) },
            context,
            undefined,
            undefined,
        ...
        expect(evidence).not.toBeNull();
        expect(evidence!.perMetricContributions.readiness).toBe(0);
        expect(evidence!.contribution).toBeGreaterThanOrEqual(0);
        expect(Object.values(evidence!.perMetricContributions).every(value => value >= 0)).toBe(true);
    });

    it('can record a non-zero signal that is not decision-relevant at the current thresholds', () => {
        const evidence = buildSubjectiveDriftDecisionEvidence(
            { subjective, objective, subjectiveBaseline: baseline(0.05) },
            context,
        );
        expect(evidence).not.toBeNull();
        expect(evidence!.contribution).toBeGreaterThan(0);
        expect(evidence!.modeWithoutDrift).toBe('train');
        expect(evidence!.modeWithDrift).toBe('train');
        expect(evidence!.decisionRelevant).toBe(false);
    });
});

describe('compactSubjectiveDriftAudit', () => {
    it('drops counterfactual-only fields and retains bounded normalized provenance', () => {
        const evidence = buildSubjectiveDriftDecisionEvidence(
            { subjective, objective, subjectiveBaseline: baseline(0.25) },
            context,
        );
