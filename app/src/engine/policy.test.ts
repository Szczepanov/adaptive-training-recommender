...
describe('isHistoricalPolicyVersion', () => {
    it('records the current transition and the previous live policies exactly once', () => {
        expect(POLICY_VERSION).toBe('2026-09-perf-engine-and-ui-lookup-optimizations-v1');
        expect(HISTORICAL_POLICY_VERSIONS[0]).toBe('2026-09-pre-compute-map-for-static-array-lookup-v1');
    });
