...
describe('isHistoricalPolicyVersion', () => {
    it('records the current transition and the previous live policies exactly once', () => {
        expect(POLICY_VERSION).toBe('2026-09-neuromuscular-power-maintenance-v1');
        expect(HISTORICAL_POLICY_VERSIONS[0]).toBe('2026-09-olympic-triathlon-plan-taper-budget-v1');
        expect(HISTORICAL_POLICY_VERSIONS[1]).toBe('2026-09-evergreen-quality-transactional-substitution-v1');
        ...
        expect(HISTORICAL_POLICY_VERSIONS.filter(
            (version) => version === '2026-09-modify-tier-aerobic-coverage-specificity-v1',
        )).toHaveLength(1);
        expect(HISTORICAL_POLICY_VERSIONS.filter(
