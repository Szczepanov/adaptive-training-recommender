...
import type { SubjectiveDriftAudit } from './subjectiveDriftEvidence';
...
function driftAudit(overrides: Partial<SubjectiveDriftAudit> = {}): SubjectiveDriftAudit {
    return {
        ...
        perMetricContributions: {
            readiness: 0.2, sleepQuality: 0.2, fatigue: 0.2,
            soreness: 0.2, mentalStress: 0.2, motivation: 0.2,
        },
...
function recommendation(subjectiveDrift?: unknown): DailyRecommendation {
    const baseAudit: RecommendationAudit = {
        ...
        decisionContextRevision: 'history-v1:test',
        ...
        candidateScores: [{ templateId: 'easy-ride', utilityScore: 1, excludedReasons: [] }],
        droppedContributorObjectives: [],
    };
    const audit = subjectiveDrift === undefined
        ? baseAudit
        : ({ ...baseAudit, subjectiveDrift } as RecommendationAudit);
    return {
        userId: 'u1', date: '2026-08-16',
        templateId: 'easy-ride', templateTitle: 'Easy ride', category: 'Easy Endurance', modality: 'Cycling',
        mode: 'modify', rationale: 'test', schemaVersion: 3, createdAt: '', updatedAt: '',
...
describe('Phase 9.7 subjective drift replay', () => {
    it('preserves legacy replay when subjective drift provenance is absent', () => {
        expect(replayRecommendationAudit(recommendation())).toEqual({
            reproducible: true,
    ...
    it('rejects non-reconciling components and non-canonical metric maps', () => {
        const nonReconciling = driftAudit({
            contribution: 9,
            perMetricContributions: {
                readiness: 0.2, sleepQuality: 0.2, fatigue: 0.2,
                soreness: 0.2, mentalStress: 0.2, motivation: 0.2,
