...
export const SWIMMING_WORKOUTS: WorkoutDefinition[] = [
  {
    ...
    blocks: [
      { id: 'warmup', name: 'Easy warm-up', role: 'warmup', steps: [
        timeStep('swim_tech_warmup', 'swim_easy_continuous', 'Easy warm-up swimming', 480, { target: { type: 'rpe', min: 1, max: 2 } })
      ]},
      ...
      { id: 'cooldown', name: 'Easy cool-down', role: 'cooldown', steps: [
        timeStep('swim_tech_cooldown', 'swim_easy_continuous', 'Easy cool-down swimming', 300, { target: { type: 'rpe', min: 1, max: 2 } })
      ]}
    ],
    variants: [
      ...
      { id: 'reduced', targetDurationMin: 28, loadMultiplier: 0.75, rationale: 'Reduce drill repeats while preserving the technical theme.', stepOverrides: [{ stepId: 'swim_tech_drill', sets: 4 }] },
      { id: 'return_to_training', targetDurationMin: 25, loadMultiplier: 0.5, rationale: 'Use mostly easy continuous swimming with minimal drill work.', stepOverrides: [{ stepId: 'swim_tech_drill', sets: 2 }, { stepId: 'swim_tech_cooldown', durationSeconds: 240 }] }
    ],
    parameters: [
  ...
  },
  {
    ...
    blocks: [
      { id: 'warmup', name: 'Warm-up', role: 'warmup', steps: [
        timeStep('swim_easy_warmup', 'swim_easy_continuous', 'Easy warm-up swimming', 360, { target: { type: 'rpe', min: 1, max: 2 } })
      ]},
    ...
    variants: [
      ...
      { id: 'reduced', targetDurationMin: 35, loadMultiplier: 0.75, rationale: 'Retain the aerobic stimulus while lowering total volume.', stepOverrides: [{ stepId: 'swim_easy_main', durationSeconds: 1320 }] },
      { id: 'return_to_training', targetDurationMin: 30, loadMultiplier: 0.55, rationale: 'Use a conservative relaxed exposure.', stepOverrides: [{ stepId: 'swim_easy_main', durationSeconds: 900 }, { stepId: 'swim_easy_cooldown', durationSeconds: 240 }] }
    ],
    parameters: [
  ...
  },
  {
    ...
    blocks: [
      { id: 'warmup', name: 'Warm-up', role: 'warmup', steps: [
        timeStep('swim_thresh_warmup', 'swim_easy_continuous', 'Easy warm-up swimming', 480, { target: { type: 'rpe', min: 1, max: 2 } })
      ]},
      { id: 'main', name: 'Threshold repeats', role: 'main', steps: [
        timeStep('swim_thresh_main', 'swim_threshold_interval', 'Sustained swim interval', 240, { sets: 5, restAfterSec: 45, target: { type: 'rpe', min: 6, max: 7 }, notes: ['Keep the final repeat consistent with the first rather than fading.', 'Prioritise repeat consistency over chasing a pace target.'] })
      ]},
      { id: 'cooldown', name: 'Cool-down', role: 'cooldown', steps: [
        timeStep('swim_thresh_cooldown', 'swim_easy_continuous', 'Easy cool-down swimming', 300, { target: { type: 'rpe', min: 1, max: 2 } })
      ]}
    ],
    variants: [
      { id: 'full', targetDurationMin: 45, loadMultiplier: 1, rationale: 'Five repeatable threshold intervals.', stepOverrides: [] },
      { id: 'reduced', targetDurationMin: 38, loadMultiplier: 0.75, rationale: 'Reduce interval count before increasing effort.', stepOverrides: [{ stepId: 'swim_thresh_main', sets: 4 }] },
      { id: 'return_to_training', targetDurationMin: 35, loadMultiplier: 0.55, rationale: 'Use fewer, easier repeats.', stepOverrides: [{ stepId: 'swim_thresh_main', sets: 3, target: { type: 'rpe', min: 4, max: 5 } }] }
    ],
    parameters: [
