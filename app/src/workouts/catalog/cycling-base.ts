...
export const CYCLING_BASE_WORKOUTS: WorkoutDefinition[] = [
  {
      ...
      blocks: [
        { id: 'main', name: 'Easy continuous spin', role: 'main', steps: [
          timeStep('easy_spin', 'bike_easy_spin', 'Easy spin', 1200, { target: { type: 'rpe', min: 1, max: 2 }, notes: ['Smooth cadence', 'No low-cadence grinding'] })
        ]},
        { id: 'cooldown', name: 'Downshift', role: 'cooldown', steps: [
          timeStep('downshift', 'bike_easy_spin', 'Very easy finish', 600, { target: { type: 'rpe', min: 1, max: 1 } })
        ]}
      ],
      variants: [
      ...
      regressions: [], progressions: ['cycling_zone2_standard_01'], substitutions: [],
      garmin: { exportable: true, supportedSport: 'cycling' },
      tags: ['low_fatigue', 'indoor_or_outdoor', 'recovery'],
    ...
    },
  {
      ...
      duration: { defaultMin: 60, minimumMin: 40, maximumMin: 90 },
      ...
      blocks: [
        { id: 'warmup', name: 'Progressive warm-up', role: 'warmup', steps: [
          timeStep('warmup_easy', 'bike_progressive_warmup', 'Progressive easy riding', 600, { target: { type: 'rpe', min: 1, max: 2 } })
        ]},
        { id: 'main', name: 'Zone 2 base', role: 'main', steps: [
          timeStep('zone2_main', 'bike_easy_spin', 'Steady Zone 2', 2400, { target: { type: 'rpe', min: 2, max: 3 }, notes: ['Conversational breathing', 'Usually 85–95 rpm', 'Power and heart rate should remain stable'] })
        ]},
        { id: 'cooldown', name: 'Cool-down', role: 'cooldown', steps: [
          timeStep('cooldown_easy', 'bike_easy_spin', 'Easy finish', 600, { target: { type: 'rpe', min: 1, max: 2 } })
        ]}
      ],
      variants: [
      ...
      regressions: ['cycling_recovery_spin_01'], progressions: ['cycling_controlled_threshold_4x8_01'], substitutions: [],
      garmin: { exportable: true, supportedSport: 'cycling' },
      tags: ['aerobic_base', 'low_impact', 'indoor_or_outdoor'],
