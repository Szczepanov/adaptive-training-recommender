...
function validateTarget(target: IntensityTarget, path: string, errors: string[]): void {
  if (target.type === 'rpe' && (target.min < 1 || target.max > 10 || target.min > target.max)) {
    errors.push(`${path}: invalid RPE range ${target.min}-${target.max}`);
  }
  if (target.type === 'heart_rate_zone' && (target.zone < 1 || target.zone > 7)) {
    errors.push(`${path}: heart-rate zone must be between 1 and 7`);
  }
  if (target.type === 'power_zone' && (target.zone < 1 || target.zone > 7)) {
    errors.push(`${path}: power zone must be between 1 and 7`);
  }
  if (target.type === 'ftp_percent' && (target.min <= 0 || target.max > 200 || target.min > target.max)) {
    errors.push(`${path}: invalid FTP percentage range ${target.min}-${target.max}`);
  }
  if (target.type === 'cadence' && (target.minRpm <= 0 || target.maxRpm > 250 || target.minRpm > target.maxRpm)) {
    errors.push(`${path}: invalid cadence range ${target.minRpm}-${target.maxRpm}`);
  }
  if (target.type === 'reps_in_reserve' && (target.min < 0 || target.max > 10 || target.min > target.max)) {
    errors.push(`${path}: invalid repetitions-in-reserve range ${target.min}-${target.max}`);
  }
}
...
export function validateWorkoutLibrary(
  ...
  for (const workout of workouts) {
    ...
    if (workout.duration.minimumMin > workout.duration.defaultMin || workout.duration.defaultMin > workout.duration.maximumMin) {
      errors.push(`${prefix}: duration must satisfy minimum <= default <= maximum`);
    }
    ...
    for (const expectedId of expectedVariantIds) {
      if (!variantIds.has(expectedId as 'full' | 'reduced' | 'return_to_training')) {
        errors.push(`${prefix}: missing ${expectedId} variant`);
      }
    }
    ...
    for (const variant of workout.variants) {
      if (variant.targetDurationMin <= 0) errors.push(`${prefix}/${variant.id}: targetDurationMin must be positive`);
      if (variant.loadMultiplier <= 0 || variant.loadMultiplier > 1.2) {
        errors.push(`${prefix}/${variant.id}: loadMultiplier must be greater than 0 and no more than 1.2`);
      }
    ...
    if (workout.garmin.exportable && workout.modality !== 'cycling' && workout.modality !== 'running') {
      errors.push(`${prefix}: only cycling and running workouts may be Garmin-exportable`);
    }
    if (workout.garmin.exportable && workout.garmin.supportedSport !== workout.modality) {
      errors.push(`${prefix}: Garmin supportedSport must match workout modality`);
    }
  }
