...
def test_returns_none_for_a_freeform_recording_with_no_workout_evidence():
    assert compute_fit_workout_fingerprint(None, ()) is None
    assert compute_fit_workout_fingerprint("", ()) is None


def test_returns_a_versioned_fingerprint_when_workout_evidence_is_present():
...
def test_is_order_independent_over_step_indices():
    a = compute_fit_workout_fingerprint("5x5 Squat", (2, 0, 1))
    b = compute_fit_workout_fingerprint("5x5 Squat", (0, 1, 2))
    assert a == b


def test_differs_for_a_different_workout_name():
    a = compute_fit_workout_fingerprint("5x5 Squat", (0, 1))
    b = compute_fit_workout_fingerprint("5x5 Bench", (0, 1))
    assert a != b


def test_differs_for_a_different_step_structure():
    a = compute_fit_workout_fingerprint("5x5 Squat", (0, 1))
    b = compute_fit_workout_fingerprint("5x5 Squat", (0, 1, 2))
    assert a != b


def test_produces_a_fingerprint_from_step_indices_alone_with_no_workout_name():
