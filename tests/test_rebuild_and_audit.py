...
def test_rebuild_reproduces_snapshot_from_archive_without_garmin_calls(tmp_path: Path) -> None:
...
def test_rebuild_preserves_existing_firestore_history_on_skipped_dates(tmp_path: Path) -> None:
    archive_store = LocalRawArchiveStore(base_dir=tmp_path)
