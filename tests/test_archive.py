...
def test_local_archive_round_trip(tmp_path) -> None:
...
def test_local_archive_idempotent_skip_on_identical_payload(tmp_path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
...
def test_local_archive_different_payload_same_date_not_skipped(tmp_path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
...
def test_local_archive_load_missing_returns_none(tmp_path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
    assert store.load("stats", "2026-08-06") is None


def test_local_archive_list_archived_dates_range_filter(tmp_path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
...
def test_null_archive_store_is_always_a_noop(tmp_path) -> None:
...
def test_archive_rejects_path_traversal_identifiers(tmp_path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
