...
def test_local_archive_round_trip(tmp_path: Path) -> None:
    ...
    record = ArchiveRecord("stats", "2026-08-06", payload, "run-1", "0.3.8")
    object_path = store.archive(record)
...
def test_local_archive_idempotent_skip_on_identical_payload(tmp_path: Path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
...
def test_local_archive_different_payload_same_date_not_skipped(tmp_path: Path) -> None:
    store = LocalRawArchiveStore(base_dir=tmp_path)
