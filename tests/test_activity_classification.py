...
def test_classify_activity_falls_back_when_zone4_floor_invalid():
    ...
    is_hard_none, tag_none = classify_activity_intensity(
        training_effect=2.0, average_hr=145, zone4_floor=None
    )
    assert is_hard_none is True
    assert tag_none == "hard"

    is_hard_zero, tag_zero = classify_activity_intensity(
        training_effect=2.0, average_hr=145, zone4_floor=0
    )
    assert is_hard_zero is True
    assert tag_zero == "hard"

    is_hard_neg, tag_neg = classify_activity_intensity(
        training_effect=2.0, average_hr=145, zone4_floor=-5
    )
